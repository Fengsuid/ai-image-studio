const state = {
  lang: window.AppModules?.settings?.readPreference?.("lang", "zh") || "zh",
  user: null,
  settings: null,
  firstRun: false,
  view: "home",
  ...
  continuationMode: window.AppModules?.settings?.readPreference?.("imageStudio.continuationMode", "auto") || "auto",
...
function settingsControllerContext() {
  return { state, elements, $$ };
}

function requireSettingsController() {
  if (settingsController) return settingsController;
  const create = window.AppModules?.settings?.create;
  if (typeof create !== "function") throw new Error("Settings module has not been loaded");
  settingsController = create(settingsControllerContext());
  return settingsController;
}

function initSettingsController() {
  const controller = requireSettingsController();
  window.AppModules?.register?.("settings", {
    ...controller,
    updateNav,
    syncThemeMobileNav
  });
  window.ImageStudioText = controller.text;
  return controller;
}
...
function authControllerContext() {
  return {
    ...
    imageFallbackImgAttrs, imageVariantUrl, setView, syncComposers, submitGeneration, openImageEditor, openPublishModal,
    publicKindTagForItem, publicTagsForKind, loadPublicGallery, conversationRouteForItem, formatElapsed, isCanvasEntryHidden,
    copyText, openCanvasTargetModal, canvasPayloadFromGeneration
  };
}

function requireAuthController() {
  if (authController) return authController;
  const create = window.AppModules?.auth?.create;
  if (typeof create !== "function") throw new Error("Auth module has not been loaded");
  authController = create(authControllerContext());
  return authController;
}

function initAuthController() {
  const controller = requireAuthController();
  window.AppModules?.register?.("auth", controller);
  return controller;
}

function galleryControllerContext() {
  return {
    $,
    $$,
    api,
...
function requireGalleryController() {
  if (galleryController) return galleryController;
  const create = window.AppModules?.gallery?.createController;
  if (typeof create !== "function") throw new Error("Gallery module has not been loaded");
  galleryController = create(galleryControllerContext());
...
function wireGalleryImageCache(...args) {
  return requireGalleryController().wireGalleryImageCache(...args);
}
...
function galleryTagViewModelForItem(...args) {
  return requireGalleryController().galleryTagViewModelForItem(...args);
}
...
function applyI18n(root = document) {
  return requireSettingsController().applyI18n(root);
}
function updateDailyMetric() {
  return requireSettingsController().updateDailyMetric();
}
function updateNav() {
...
function openAuthModal(mode = state.authMode) { return requireAuthController().openAuthModal(mode); }
async function openCreditsModal() { return requireAuthController().openCreditsModal(); }
...
function isHistorySessionTitle(title = "") {
  return requireSettingsController().matchesText("historySession", title);
}
...
async function unpublishGeneration(item) {
  ...
  const { message } = requireAuthController().withdrawalPromptForItem(item);
...
function renderExamples(...args) {
  return requireGalleryController().renderExamples(...args);
}
function existingPublishTagChoices(...args) {
  return requireGalleryController().existingPublishTagChoices(...args);
}
function promptLibraryModule(...args) {
  return requireGalleryController().promptLibraryModule(...args);
}
function promptLibraryRenderContext(...args) {
  return requireGalleryController().promptLibraryRenderContext(...args);
}
function renderLibrary(...args) {
  return requireGalleryController().renderLibrary(...args);
}
function renderLeaderboardPage(...args) {
  return requireGalleryController().renderLeaderboardPage(...args);
}
...
function squareItemFromPrompt(...args) {
  return requireGalleryController().squareItemFromPrompt(...args);
}
function openGalleryUnavailableModal(...args) {
  return requireGalleryController().openGalleryUnavailableModal(...args);
}
function openSquarePreviewById(...args) {
  return requireGalleryController().openSquarePreviewById(...args);
}
...
function loadPublicGallery(...args) {
  return requireGalleryController().loadPublicGallery(...args);
}
function loadGalleryLeaderboard(...args) {
  return requireGalleryController().loadGalleryLeaderboard(...args);
}
function ensureGalleryLeaderboardLoaded(...args) {
  return requireGalleryController().ensureGalleryLeaderboardLoaded(...args);
}
...
function togglePromptLike(...args) {
  return requireGalleryController().togglePromptLike(...args);
}
function toggleGalleryLike(...args) {
  return requireGalleryController().toggleGalleryLike(...args);
}
...
function openComplianceNotice() {
  ...
  openModal(`
    <section class="modal compliance-modal" role="dialog" aria-modal="true" aria-labelledby="complianceTitle">
      ...
      <div class="compliance-title"><h2 id="complianceTitle"><i class="ri-megaphone-fill"></i>${text("noticeTitle")}</h2><p>${text("noticeSubtitle")}</p></div>
      <div class="notice-card danger"><h3><span></span>${text("noticeCore")}</h3><ul><li>平台严禁低俗色情、暴力血腥、网络诈骗、政治敏感及其他违规内容。</li><li>系统已启用内容安全审计，违规账号将被限制、封禁或销号。</li></ul></div>
      <div class="notice-card privacy"><h3><i class="ri-shield-user-line"></i>${text("noticePrivacy")}</h3><p>用户数据仅用于系统运行、合规与安全保障，不出售或披露给第三方。</p></div>
      <div class="notice-card together"><p>${text("noticeTogether")}</p></div>
      <div class="compliance-actions"><button class="modal-primary" type="button" data-compliance-ack>${text("noticeAck")}</button></div>
    </section>
  `);
  const markSeen = () => { localStorage.setItem(storageKey, "seen"); closeModal(); };
  $("[data-compliance-ack]", elements.modalLayer)?.addEventListener("click", markSeen);
  $(".compliance-close", elements.modalLayer)?.addEventListener("click", () => localStorage.setItem(storageKey, "seen"));
}
function openMyWorksModal(options = {}) {
  return requireAuthController().openMyWorksModal(options);
}
function openWorkDetail(id, options = {}) {
  return requireAuthController().openWorkDetail(id, options);
}
