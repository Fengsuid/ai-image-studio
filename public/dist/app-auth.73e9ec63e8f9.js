(function(w){"use strict";const S=w.AppModules||(w.AppModules={}),p=(r,e=document)=>e.querySelector(r),m=(r,e=document)=>Array.from(e.querySelectorAll(r));function U(r){const e=document.cookie.split(";").map(l=>l.trim()).filter(Boolean).map(l=>l.split("=")).find(([l])=>decodeURIComponent(l)===r);return e?decodeURIComponent(e.slice(1).join("=")):""}function b(r,e){const l=r[e];if(l==null)throw new Error(`AppModules.auth missing context: ${e}`);return l}function K(r={}){const e=b(r,"state"),l=b(r,"elements"),s=b(r,"text"),n=b(r,"escapeHtml"),I=b(r,"formatDate"),F=b(r,"truncate"),X=b(r,"maskContactEmail"),C=b(r,"openModal"),f=b(r,"closeModal"),g=b(r,"showToast");async function y(i,a={}){const t=String(a.method||"GET").toUpperCase(),c={"Content-Type":"application/json",...a.headers||{}};["GET","HEAD","OPTIONS"].includes(t)||(c["X-CSRF-Token"]=e.csrfToken||U("csrf"));const d=await fetch(i,{...a,credentials:"same-origin",headers:c});if(d.status===204)return null;const u=await d.json().catch(()=>({}));if(u.csrfToken&&(e.csrfToken=u.csrfToken),!d.ok){const v=(u.details?.requiredMode||u.details?.audit?.requiredMode||"")==="image-to-image"?e.lang==="zh"?"提示词与已有公开内容高度相似，请改用图生图或含原图发布。":"This prompt is highly similar to existing public content. Publish it as image-to-image instead.":u.error||"Request failed",o=new Error(v);throw o.status=d.status,o.details=u.details||null,o}return u}function E(){l.accountMenu?.classList.add("hidden"),l.accountMenuBtn?.setAttribute("aria-expanded","false")}function L(i=e.authMode){e.authMode=i;const a=i==="register";C(`
        <section class="modal">
          ...
          <div class="auth-tabs" role="tablist" aria-label="${n(s(a?"registerTitle":"loginTitle"))}">
            <button type="button" role="tab" aria-selected="${a?"false":"true"}" class="${a?"":"active"}" data-auth-mode="login">${s("submitLogin")}</button>
            <button type="button" role="tab" aria-selected="${a?"true":"false"}" class="${a?"active":""}" data-auth-mode="register">${s("submitRegister")}</button>
          </div>
      ...
      `),m("[data-auth-mode]",l.modalLayer).forEach(t=>{t.addEventListener("click",()=>L(t.dataset.authMode))}),p("[data-close-auth]",l.modalLayer).addEventListener("click",f),p("#authForm").addEventListener("submit",B)}async function B(i){i.preventDefault();const a=i.currentTarget.querySelector("button[type='submit']");a.disabled=!0;try{const t={email:p("#authEmail").value,password:p("#authPassword").value,name:p("#authName")?.value||""},c=await y(`/api/auth/${e.authMode}`,{method:"POST",body:JSON.stringify(t)});if(c.pendingApproval){g(e.lang==="zh"?"账号已创建，等待管理员启用":"Account created, waiting for approval","ri-time-line"),f();return}e.user=c.user,r.setCurrentCacheUser();const d=await y("/api/auth/me");e.settings=d.settings,e.firstRun=d.firstRun,e.checkin=d.checkin||e.checkin,await r.loadHistory(),r.ensureImageSessions(),await r.loadAnnouncements(),f();const u=e.pendingAuthView;e.pendingAuthView="",e.forceHero=!u,r.renderAll(),u==="canvas-v2"?w.location.assign(r.canvasV2ProjectUrl()):u&&r.navigate(u,{scrollTop:!0}),setTimeout(r.maybeOpenUnreadAnnouncementModal,300),w.scrollTo({top:0,behavior:"auto"}),r.restartHeroVideo()}catch(t){g(t.message,"ri-error-warning-line")}finally{a.disabled=!1}}async function H(){const i=e.user?.id||e.user?.email||"";await y("/api/auth/logout",{method:"POST"}).catch(()=>null),await r.cacheDb()?.clearUserCache?.(i),e.user=null,r.setCurrentCacheUser(null),e.history=[],e.imageSessions=[],e.activeImageSessionId="",e.announcements=[],e.unreadAnnouncements=[],e.notificationModalShown.clear(),e.checkin={checkedInToday:!1,credit:e.settings?.checkinCredit||1},e.forceHero=!0,r.renderAll(),w.scrollTo({top:0,behavior:"auto"}),r.restartHeroVideo()}async function O(){try{return await y("/api/credits/detail?limit=80")}catch(i){return console.warn("[credits]",i),{ledger:[],rewards:[]}}}async function A(){if(!e.user){L("login");return}const i=await O();C(w.ImageStudioCreditsDetail.renderModal({details:i,state:e,helpers:{escapeHtml:n,text:s,formatDate:I}})),p("[data-checkin]",l.modalLayer)?.addEventListener("click",M),p("[data-close-auth]",l.modalLayer).addEventListener("click",f)}async function M(i){const a=i.currentTarget;a.disabled=!0;try{const t=await y("/api/checkin",{method:"POST"});e.user=t.user||{...e.user,credits:t.credits},r.setCurrentCacheUser(),e.checkin=t.checkin||{checkedInToday:!0,credit:e.checkin?.credit||1},g(t.checkedIn?e.lang==="zh"?`签到成功，获得 ${t.awarded} 积分`:`Checked in, +${t.awarded} credit`:s("checkedIn"),"ri-calendar-check-line"),r.updateNav(),A()}catch(t){g(t.message,"ri-error-warning-line"),a.disabled=!1}}function P(){const i=String(e.settings?.contactEmail??e.settings?.contactAdminEmail??"").trim();if(!i)return;const a=`mailto:${i}?subject=${encodeURIComponent("ai-image-studio support")}`,t=X(i);C(`
        <section class="modal">
          <button class="close-modal" type="button"><i class="ri-close-line"></i></button>
          <div class="modal-title">
      ...
      `),p("[data-copy-contact-email]",l.modalLayer).addEventListener("click",async()=>{typeof r.copyText=="function"?await r.copyText(i):await navigator.clipboard?.writeText(i),g(s("contactCopied"),"ri-file-copy-line")}),p("[data-close-auth]",l.modalLayer).addEventListener("click",f)}function q(i={}){if(!e.user){L("login");return}r.syncThemeMobileNav("works");const a=i.replaceRoute!==!1;e.worksFilter=e.worksFilter||"all",e.worksDateFilter=e.worksDateFilter||"all",e.worksTagFilter=e.worksTagFilter||"all";const t=[{id:"all",label:s("worksFilterAll")},{id:"public",label:s("worksFilterPublic")},{id:"private",label:s("worksFilterPrivate")},{id:"text",label:s("worksFilterText")},{id:"image",label:s("worksFilterImage")},{id:"archived",label:s("worksFilterArchived")}],c=Q();if(C(`
        <section class="modal works-modal works-workspace">
          ...
          <div class="works-filter-bar" role="tablist" aria-label="${n(e.lang==="zh"?"作品类型筛选":"Work type filters")}">
            ${t.map(d=>`<button type="button" role="tab" aria-selected="${e.worksFilter===d.id?"true":"false"}" data-works-filter="${d.id}" class="works-filter-btn${e.worksFilter===d.id?" active":""}">${n(d.label)}</button>`).join("")}
          </div>
      ...
      `),p("[data-works-refresh]",l.modalLayer).addEventListener("click",()=>$(!0)),p("#worksSearchInput",l.modalLayer).addEventListener("input",d=>{e.worksSearch=d.target.value,$(!1)}),p("#worksDateFilter",l.modalLayer).addEventListener("change",d=>{e.worksDateFilter=d.target.value||"all",e.worksSelected.clear(),$(!1)}),p("#worksTagFilter",l.modalLayer).addEventListener("change",d=>{e.worksTagFilter=d.target.value||"all",e.worksSelected.clear(),$(!1)}),m("[data-works-filter]",l.modalLayer).forEach(d=>{d.addEventListener("click",()=>{e.worksFilter=d.dataset.worksFilter||"all",m("[data-works-filter]",l.modalLayer).forEach(u=>{const k=u===d;u.classList.toggle("active",k),u.setAttribute("aria-selected",k?"true":"false")}),$(!1)})}),m("[data-works-bulk]",l.modalLayer).forEach(d=>{d.addEventListener("click",()=>x(d.dataset.worksBulk))}),$(!1),a&&!e.routeSyncing){const d=r.routeState({modal:"works"});w.history?.pushState?.(d,"",r.routeUrl(d))}}async function $(i=!1){const a=p("#worksGrid",l.modalLayer);if(!a)return;a.innerHTML=`<div class="empty-message">${s("loadingPrompts")}</div>`,i&&await r.loadHistory();const t=e.worksFilter||"all",c=String(e.worksSearch||"").trim().toLowerCase(),d=e.worksDateFilter||"all",u=String(e.worksTagFilter||"all"),k=_(d),v=[...e.history].filter(o=>o.status==="done"&&o.images?.[0]).filter(o=>{const h=r.isImageToImageItem(o);switch(t){case"public":return!!o.isPublic&&!o.archived;case"private":return!o.isPublic&&!o.archived;case"text":return!h&&!o.archived;case"image":return h&&!o.archived;case"archived":return!!o.archived;default:return!o.archived}}).filter(o=>{if(!k)return!0;const h=new Date(o.time||0).getTime();return Number.isFinite(h)&&h>=k}).filter(o=>u==="all"?!0:(o.publicTags||[]).some(h=>j(h)===u)).filter(o=>c?[o.prompt,I(o.time),...(o.publicTags||[]).map(r.displayTag)].some(h=>String(h||"").toLowerCase().includes(c)):!0).sort((o,h)=>new Date(h.time||0)-new Date(o.time||0));if(V(),!v.length){a.innerHTML=`<div class="empty-message">${s(t==="all"?"emptyWorks":"worksFilterEmpty")}</div>`;return}a.innerHTML=v.map(o=>{const h=!!(o.sourceImageData||o.sourceImageUrl),T=r.isImageToImageItem(o),ie=o.publicTags?.length?` · ${o.publicTags.map(r.displayTag).join(" / ")}`:"",J=o.isPublic?r.publicRewardLabel(o):"",re=`
        ...
        <article class="work-card${o.archived?" archived":""}" data-work-id="${n(o.id)}" tabindex="0" role="button" aria-label="${n(s("worksOpenDetail"))}">
          <div class="work-visual" data-work-detail="${n(o.id)}" ${r.imageFallbackContainerAttrs()}>
            ...
            <img src="${n(r.imageVariantUrl(o.images[0]))}" ${r.imageFallbackImgAttrs()} loading="lazy" decoding="async" alt="${n(F(o.prompt,80))}">
      ...
      `}).join(""),Y(a)}function _(i){const a=Date.now();if(i==="today"){const t=new Date;return t.setHours(0,0,0,0),t.getTime()}return i==="7d"?a-7*24*60*60*1e3:i==="30d"?a-30*24*60*60*1e3:0}function j(i){return i&&typeof i=="object"?String(i.slug||i.id||i.label||i.name||"").trim():String(i||"").trim()}function Q(){const i=new Map;for(const a of e.history||[])for(const t of a.publicTags||[]){const c=j(t);c&&i.set(c,r.displayTag(t))}return[...i.entries()].map(([a,t])=>({value:a,label:t||a})).sort((a,t)=>a.label.localeCompare(t.label,e.lang==="zh"?"zh-Hans-CN":"en"))}function Y(i){m("[data-work-select]",i).forEach(a=>{a.addEventListener("click",t=>t.stopPropagation()),a.addEventListener("change",()=>{a.checked?e.worksSelected.add(String(a.dataset.workSelect)):e.worksSelected.delete(String(a.dataset.workSelect)),V()})}),m("[data-work-detail]",i).forEach(a=>{a.addEventListener("click",t=>{t.stopPropagation(),D(a.dataset.workDetail)})}),m(".work-card",i).forEach(a=>{const t=()=>D(a.dataset.workId);a.addEventListener("click",t),a.addEventListener("keydown",c=>{(c.key==="Enter"||c.key===" ")&&(c.preventDefault(),t())})}),m("a, button",i).forEach(a=>{a.hasAttribute("data-work-detail")||a.addEventListener("click",t=>t.stopPropagation())}),m("[data-work-retry]",i).forEach(a=>{a.addEventListener("click",()=>{const t=e.history.find(c=>String(c.id)===a.dataset.workRetry);t&&(f(),e.forceHero=!0,e.draftPrompt=t.prompt,r.setView("home"),r.syncComposers(),setTimeout(()=>r.submitGeneration(p(".composer",l.heroComposerMount)),80))})}),m("[data-work-editor]",i).forEach(a=>{a.addEventListener("click",()=>{const t=e.history.find(c=>String(c.id)===a.dataset.workEditor);t?.images?.[0]&&(f(),r.openImageEditor(t.images[0],t.prompt))})}),m("[data-work-publish]",i).forEach(a=>{a.addEventListener("click",()=>{const t=e.history.find(c=>String(c.id)===a.dataset.workPublish);t&&r.openPublishModal(t,!1)})}),m("[data-work-publish-original]",i).forEach(a=>{a.addEventListener("click",()=>{const t=e.history.find(c=>String(c.id)===a.dataset.workPublishOriginal);t&&r.openPublishModal(t,!0)})}),m("[data-work-withdraw]",i).forEach(a=>{a.addEventListener("click",()=>G(a.dataset.workWithdraw))})}function Z(){return Math.max(1,Number(e.settings?.publicWithdrawalWindowHours||12))}function N(i){const a=Z(),t=!i.publishedAt||Date.now()-new Date(i.publishedAt).getTime()<=a*60*60*1e3,c=e.lang==="zh"?`${a} 小时`:`${a} hour${a===1?"":"s"}`;return{withinWindow:t,message:t?e.lang==="zh"?`确认撤回公开？${c}内撤回会取消未入账奖励。`:`Unpublish this work? Pending reward will be cancelled within ${c}.`:e.lang==="zh"?`已超过 ${c}，将提交撤回申请。`:`More than ${c} passed. This will submit a withdrawal request.`}}async function G(i){if(!r.canUserUnpublishPublicWork()){g(e.lang==="zh"?"已关闭用户取消公开功能，请联系管理员处理。":"User unpublish is disabled; contact an admin.","ri-lock-line");return}const a=e.history.find(d=>String(d.id)===String(i));if(!a)return;const{withinWindow:t,message:c}=N(a);if(confirm(c))try{await y(`/api/images/${encodeURIComponent(i)}/withdrawal`,{method:"POST",body:JSON.stringify({reason:"user_request"})}),await r.loadHistory(),await r.loadPublicGallery(),$(!1),g(t?s("unpublishDone"):e.lang==="zh"?"撤回申请已提交":"Withdrawal request submitted","ri-checkbox-circle-line")}catch(d){g(d.message,"ri-error-warning-line")}}function V(){const i=p("[data-works-selection]",l.modalLayer);i&&(i.textContent=`${e.worksSelected.size} ${s("worksSelected")}`)}async function x(i){const a=Array.from(e.worksSelected);if(!a.length)return;if(i==="export"){await te(a);return}const t=i==="unpublish"||i==="archive"||i==="delete",c={publish:e.lang==="zh"?"公开":"publish",unpublish:e.lang==="zh"?"撤回公开":"unpublish",archive:e.lang==="zh"?"归档":"archive",unarchive:e.lang==="zh"?"取消归档":"unarchive",delete:e.lang==="zh"?"删除私有历史":"delete private history"}[i]||i,d=i==="delete"?e.lang==="zh"?`删除会把 ${a.length} 个作品移入已归档，不会清除审计文件；已公开作品会按撤回规则处理。继续？`:`Delete moves ${a.length} works to archived history and keeps audit files. Published works follow unpublish policy. Continue?`:`${e.lang==="zh"?"确认":"Confirm"}${c} ${a.length} ${e.lang==="zh"?"个作品？":"works?"}`;if(!(t&&!confirm(d)))try{const u=a.map(z).filter(Boolean),k=[...new Set(u.map(r.publicKindTagForItem))],v=k.length===1?r.publicTagsForKind(k[0],[]):[],h=((await y("/api/images/bulk",{method:"POST",body:JSON.stringify({generationIds:a,action:i,publicTags:v})})).results||[]).filter(T=>T.ok).length;e.worksSelected.clear(),await r.loadHistory(),await r.loadPublicGallery(),g(`${c}: ${h}/${a.length}`,"ri-checkbox-circle-line"),$(!1)}catch(u){g(u.message,"ri-error-warning-line")}}function z(i){return e.history.find(a=>String(a.id)===String(i))}function ee(i){const a=i.map(z).filter(t=>t?.images?.[0]);a.forEach((t,c)=>{setTimeout(()=>{const d=document.createElement("a");d.href=t.images[0],d.download=`${t.id}.png`,d.rel="noreferrer",document.body.appendChild(d),d.click(),d.remove()},c*220)}),g(`${s("worksDownloadStarted")}: ${a.length}`,"ri-download-2-line")}async function te(i){try{const t=((await y("/api/images/bulk",{method:"POST",body:JSON.stringify({generationIds:i,action:"export"})})).export?.items||[]).map(c=>c.id);ee(t.length?t:i)}catch(a){g(a.message,"ri-error-warning-line")}}function D(i,a={}){const t=z(i);if(!t?.images?.[0])return;const c=a.replaceRoute!==!1;R();const d=(t.publicTags||[]).map(r.displayTag).filter(Boolean),u=r.isImageToImageItem(t),k=t.creativeRoute?.length?t.creativeRoute:t.conversation?.length?t.conversation:r.conversationRouteForItem(t),v=window.ImageStudioReferenceImages?.renderAssetStrip?.(t.referenceAssets||[],{className:"works-detail-reference-assets",label:s("reference"),escapeHtml:n,imageFallbackContainerAttrs:r.imageFallbackContainerAttrs,imageFallbackImgAttrs:r.imageFallbackImgAttrs})||"",o=[[s("model"),t.model||"-"],[s("size"),t.options?.size||"-"],[s("quality"),t.options?.quality||"-"],[s("background"),t.options?.background||"-"],[s("format"),t.options?.outputFormat||"-"],[s("elapsed"),t.elapsedMs?r.formatElapsed(t.elapsedMs):"-"]];if(l.modalLayer.insertAdjacentHTML("beforeend",`
        ...
        <aside class="works-detail-drawer" role="dialog" aria-modal="true" aria-label="${n(s("worksDetailTitle"))}" data-work-id="${n(t.id)}">
          ...
          <div class="works-detail-stage" ${r.imageFallbackContainerAttrs()}>
            <img src="${n(t.images[0])}" ${r.imageFallbackImgAttrs()} loading="lazy" decoding="async" alt="${n(F(t.prompt,100))}">
          </div>
      ...
      `),m("[data-work-detail-close]",l.modalLayer).forEach(h=>h.addEventListener("click",R)),p("[data-work-detail-copy]",l.modalLayer)?.addEventListener("click",async()=>{await r.copyText(t.prompt||""),g(e.lang==="zh"?"提示词已复制":"Prompt copied","ri-file-copy-line")}),p("[data-work-detail-editor]",l.modalLayer)?.addEventListener("click",()=>{f(),r.openImageEditor(t.images[0],t.prompt)}),p("[data-work-detail-continue]",l.modalLayer)?.addEventListener("click",()=>{f(),e.forceHero=!0,e.draftPrompt=t.prompt,r.navigate("home"),r.syncComposers(),setTimeout(()=>p(".prompt-box",l.heroComposerMount)?.focus(),100)}),p("[data-work-detail-canvas]",l.modalLayer)?.addEventListener("click",()=>{r.openCanvasTargetModal(r.canvasPayloadFromGeneration(t,s("worksDetailTitle")))}),c&&!e.routeSyncing){const h=r.routeState({modal:"works",workDetailId:t.id});w.history?.pushState?.(h,"",r.routeUrl(h))}}function R(){if(p(".works-detail-drawer",l.modalLayer)?.remove(),p(".works-detail-backdrop",l.modalLayer)?.remove(),!e.routeSyncing&&p(".works-modal",l.modalLayer)&&w.history?.pushState){const i=r.routeState({modal:"works",workDetailId:""});w.history.replaceState(i,"",r.routeUrl(i))}}function ae(){l.contactBtn.addEventListener("click",P),l.accountEmailText?.addEventListener("click",async()=>{const i=String(e.user?.email||"").trim();i&&(typeof r.copyText=="function"?await r.copyText(i):await navigator.clipboard?.writeText(i),g(s("contactCopied"),"ri-file-copy-line"))}),l.accountEmailText?.addEventListener("keydown",i=>{["Enter"," "].includes(i.key)&&(i.preventDefault(),l.accountEmailText.click())}),l.accountContactBtn?.addEventListener("click",()=>{E(),P()}),l.accountMenuBtn?.addEventListener("click",i=>{i.stopPropagation();const a=l.accountMenu?.classList.contains("hidden");l.accountMenu?.classList.toggle("hidden",!a),l.accountMenuBtn?.setAttribute("aria-expanded",a?"true":"false")}),document.addEventListener("click",i=>{l.accountMenuWrap?.contains(i.target)||l.accountMenu?.contains(i.target)||E()}),l.loginBtn.addEventListener("click",()=>L("login")),l.topbarCheckinBtn?.addEventListener("click",M),l.topbarCreditsBtn?.addEventListener("click",A),l.logoutBtn.addEventListener("click",()=>{E(),H()}),l.creditsBtn.addEventListener("click",()=>{E(),A()}),l.myWorksBtn.addEventListener("click",()=>{E(),e.sessionDrawerLocked=!1,q()}),l.adminBtn.addEventListener("click",()=>{E(),w.location.href="/admin"})}return{api:y,closeAccountMenu:E,openAuthModal:L,submitAuth:B,logout:H,loadCreditDetails:O,openCreditsModal:A,submitCheckin:M,openContactModal:P,openMyWorksModal:q,loadMyWorks:$,openWorkDetail:D,closeWorkDetail:R,withdrawalPromptForItem:N,requestWorkWithdrawal:G,bindAccountEvents:ae}}const W={create:K,readCookie:U};typeof S.register=="function"?S.register("auth",W):S.auth=Object.freeze({...S.auth||{},...W})})(window);
