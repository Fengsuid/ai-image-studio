(function(){window.AdminModules=window.AdminModules||{},window.AdminModules.users={render({state:e,helpers:n}){const{escapeHtml:a,fmtNumber:l,fmtDate:r,toolbar:o,filtered:c,paged:b,pagination:u,emptyState:m}=n,d=c(e.users,["name","email","role","status"]),p=b(d);return`${o("搜索用户姓名、邮箱或角色",["active","disabled","admin","user"])}
        <section class="admin-panel">
          ...
          <div class="admin-bulk-bar admin-bulk-bar-polished primitive-table-bulk-bar" role="region" aria-label="用户批量操作">
            <div class="admin-bulk-summary">
              ...
              <strong>已选 ${l(e.selectedUsers.size)} 个用户</strong>
              ...
              <tbody>
                ${p.map(t=>{const i=t.firstPublicRewardStatus&&t.firstPublicRewardStatus!=="none"?{status:t.firstPublicRewardStatus,amount:t.firstPublicRewardAmount,referenceId:t.firstPublicRewardGenerationId}:e.rewardLedger.find(s=>s.userId===t.id&&s.rewardType==="first_public");return`
                    <tr>
                      ...
                      <td>${l(t.credits)}</td>
                      <td>${i?`${a(i.status)} · ${l(i.amount)}<small>${a(i.referenceId||"")}</small>`:"未发放"}</td>
                      <td>${r(t.createdAt)}</td>
