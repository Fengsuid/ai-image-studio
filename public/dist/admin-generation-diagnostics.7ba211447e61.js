(function(u){"use strict";const f=["queue","credit_reserved","provider_generation","provider_edit","provider_failed","admin"],h=["pending","running","queued","polling","succeeded","success","failed","cancelled"];function e(t){return String(t??"").replace(/[&<>"']/g,n=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[n])}function l(t){try{return e(JSON.stringify(t??{},null,2))}catch{return e(String(t??""))}}function s(t,n){return e(t.generationDiagnostics?.filters?.[n]||"")}function $(t={}){return t.providerParams?.provider||t.providerParams?.name||t.providerParams?.endpoint||"-"}function b(t={}){return t.model||t.normalizedParams?.model||t.providerParams?.model||"-"}function p(t={}){return[t.errorStage||t.failureStage||"",t.errorCode||"",t.errorMessage||""].filter(Boolean).join(" | ")}function m({state:t}){return`
      <div class="admin-toolbar generation-diagnostics-toolbar">
        ...
        <select data-generation-filter="status">
          ...
          ${h.map(n=>`<option value="${n}"${t.generationDiagnostics?.filters?.status===n?" selected":""}>${n}</option>`).join("")}
        </select>
    ...
    `}function v({state:t,helpers:n}){const o=t.generations||[],i=n.paged(o);return`${m({state:t})}
      ...
      <dl class="admin-detail-list">
        ...
        <dt>Provider</dt><dd>${e($(t))}</dd>
        <dt>模型</dt><dd>${e(b(t))}</dd>
