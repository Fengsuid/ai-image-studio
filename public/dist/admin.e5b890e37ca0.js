const adminState={user:null,version:null,settings:null,users:[],generations:[],prompts:[],promptSources:[],promptSyncRuns:[],tags:[],promptCategories:[],publicImages:[],reports:[],promptDuplicates:[],promptAudits:[],providers:[],galleryLeaderboard:[],galleryLikeAnomalies:[],galleryFileChecks:[],defaultProviderId:"",announcements:[],rum:{summary:{},events:[]},withdrawals:[],creditLedger:[],rewardLedger:[],active:location.hash.replace("#","")||"overview",search:"",status:"all",page:1,pageSize:12,generationDiagnostics:{filters:{}},selectedUsers:new Set,audit:[],csrfToken:"",sidebarCollapsed:localStorage.getItem("adminSidebarCollapsed")==="1"},navItems=[["overview","ri-dashboard-line","总览"],["providers","ri-plug-line","API 供应商"],["generation-requests","ri-image-ai-line","生成请求"],["square-review","ri-gallery-view-2","广场审核"],["gallery-files","ri-folder-warning-line","文件巡检"],["users-credits","ri-user-settings-line","用户与积分"],["prompt-cms","ri-quill-pen-line","提示词 CMS"],["prompt-audit","ri-shield-check-line","Prompt Audit"],["tag-library","ri-price-tag-3-line","标签库"],["reports-withdrawals","ri-alarm-warning-line","举报与撤回"],["growth","ri-line-chart-line","增长配置"],["announcements","ri-notification-3-line","通知公告"],["system-settings","ri-sliders-line","系统设置"],["rum-performance","ri-speed-up-line","RUM/性能"],["audit-log","ri-file-list-3-line","审计日志"]],pageDescriptions={overview:"系统健康、关键指标和最近异常。",providers:"多 API 地址、模型能力、健康检查和路由策略入口。","generation-requests":"生成任务状态、耗时、错误和请求详情。","square-review":"公开作品、举报下架和恢复到画廊。","gallery-files":"公开画廊文件缺失、文件大小和最近巡检结果。","users-credits":"用户状态、角色、积分和首发奖励流水。","prompt-cms":"提示词内容、重复候选、互动数据和人工处理。","prompt-audit":"AI 提示词重复审计、人工复核和发布门禁入口。","tag-library":"标签目录、中文展示、合并迁移和筛选展示。","reports-withdrawals":"举报、撤回申请、处理原因和审核日志。",growth:"推荐位、榜单、活动和运营增长配置。",announcements:"站内通知、登录弹窗、未读确认和公告预览。","system-settings":"注册、积分、安全和非 API Provider 类系统设置。","rum-performance":"Web Vitals、图片失败率、运行时和性能事件。","audit-log":"后台敏感操作、人工巡检和审计记录。"},$=(e,t=document)=>t.querySelector(e);function escapeHtml(e){return String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function imageVariantUrl(e,t="thumb"){if(!e||/^(data:|blob:)/i.test(e))return e||"";const a=e.includes("?")?"&":"?";return`${e}${a}variant=${encodeURIComponent(t)}`}async function api(e,t={}){const a=String(t.method||"GET").toUpperCase(),n={"Content-Type":"application/json",...t.headers||{}};["GET","HEAD","OPTIONS"].includes(a)||(n["X-CSRF-Token"]=adminState.csrfToken||readCookie("csrf"));const r=await fetch(e,{...t,credentials:"same-origin",headers:n}),s=await r.text(),i=s?JSON.parse(s):null;if(i?.csrfToken&&(adminState.csrfToken=i.csrfToken),!r.ok){const l=new Error(i?.error||i?.message||`HTTP ${r.status}`);throw l.status=r.status,l}return i}function readCookie(e){const t=document.cookie.split(";").map(a=>a.trim()).filter(Boolean).map(a=>a.split("=")).find(([a])=>decodeURIComponent(a)===e);return t?decodeURIComponent(t.slice(1).join("=")):""}function fmtNumber(e){return new Intl.NumberFormat("zh-CN").format(Number(e||0))}function fmtDate(e){if(!e)return"-";const t=new Date(e);return Number.isNaN(t.getTime())?"-":new Intl.DateTimeFormat("zh-CN",{month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"}).format(t)}function datetimeLocal(e){if(!e)return"";const t=new Date(e);if(Number.isNaN(t.getTime()))return"";const a=n=>String(n).padStart(2,"0");return`${t.getFullYear()}-${a(t.getMonth()+1)}-${a(t.getDate())}T${a(t.getHours())}:${a(t.getMinutes())}`}function fmtDuration(e){const t=Number(e||0);return t?t<1e3?`${Math.round(t)}ms`:`${(t/1e3).toFixed(t<1e4?1:0)}s`:"-"}function todayItems(e,t="createdAt"){const a=new Date;return a.setHours(0,0,0,0),e.filter(n=>new Date(n[t]||0)>=a)}function recordAudit(e,t,a=""){adminState.audit.unshift({id:`${Date.now()}-${Math.random().toString(16).slice(2)}`,action:e,target:t,detail:a,actor:adminState.user?.email||"admin",createdAt:new Date().toISOString()}),adminState.audit=adminState.audit.slice(0,80)}function generationDiagnosticsModule(){return window.AdminModules?.generationDiagnostics||null}function generationDiagnosticsQuery(){const e=adminState.generationDiagnostics?.filters||{},t=new URLSearchParams({limit:"500"});for(const[a,n]of Object.entries(e)){const r=String(n||"").trim();r&&t.set(a,r)}return`/api/admin/generations?${t.toString()}`}function generationDiagnosticsHelpers(){return{fmtNumber,fmtDate,fmtDuration,imageVariantUrl,paged,pagination}}function adminModuleContext(){return{state:adminState,helpers:{escapeHtml,fmtNumber,fmtDate,fmtDuration,imageVariantUrl,metrics,dashboardContext,requestTable,toolbar,filtered,paged,pagination,emptyState,renderPlaceholder}}}function renderAdminModule(e){return window.AdminModules?.[e]?.render?.(adminModuleContext())||""}function currentNav(){return navItems.find(([e])=>e===adminState.active)||navItems[0]}function renderNav(){document.body.classList.toggle("admin-sidebar-collapsed",adminState.sidebarCollapsed),$("#adminNav").innerHTML=navItems.map(([e,t,a])=>`
  ...
  `).join(""),$("#adminNav").querySelectorAll("button").forEach(e=>{e.addEventListener("click",()=>{adminState.active=e.dataset.section,adminState.page=1,location.hash=adminState.active,render()})})}function setStatus(e,t="neutral"){const a=$("#adminStatus");a.textContent=e,a.dataset.tone=t}function confirmAction({title:e="确认操作",message:t="该操作会立即生效。",confirmText:a="确认",danger:n=!1}={}){const r=$("#adminConfirmLayer");return r?new Promise(s=>{r.classList.remove("hidden"),r.setAttribute("aria-hidden","false"),r.innerHTML=`
    ...
    `;const i=d=>{r.classList.add("hidden"),r.setAttribute("aria-hidden","true"),r.innerHTML="",window.removeEventListener("keydown",l),s(d)},l=d=>{d.key==="Escape"&&i(!1),d.key==="Enter"&&(d.ctrlKey||d.metaKey)&&i(!0)};$("[data-confirm-cancel]",r).addEventListener("click",()=>i(!1)),$("[data-confirm-ok]",r).addEventListener("click",()=>i(!0)),window.addEventListener("keydown",l),r.addEventListener("click",function d(c){c.target===r&&(r.removeEventListener("click",d),i(!1))}),$("[data-confirm-cancel]",r)?.focus?.({preventScroll:!0})}):Promise.resolve(window.confirm(t))}async function loadAll(){setStatus("同步中");const e=await api("/api/auth/me");if(adminState.user=e.user,!adminState.user||adminState.user.role!=="admin"){setStatus("需要管理员","danger"),$("#adminContent").innerHTML=`
    ...
    `;return}const[t,a,n,r,s,i,l,d,c,u,p,b,g,h,v,o,m,y,w,f,S,A]=await Promise.all([api("/api/version"),api("/api/admin/settings"),api("/api/admin/users"),api(generationDiagnosticsQuery()),api("/api/prompts?includeHidden=1&includeNoImage=1&limit=2000"),api("/api/admin/prompt-sources?runsLimit=120"),api("/api/tags?includeHidden=1&limit=2000"),api("/api/prompt-categories?includeHidden=1"),api("/api/admin/public-images?status=queue&limit=120"),api("/api/admin/gallery-file-checks?status=broken&limit=120"),api("/api/admin/credit-ledger?limit=120"),api("/api/admin/reward-ledger?limit=120"),api("/api/admin/audit-logs?limit=120"),api("/api/admin/withdrawals?limit=120"),api("/api/admin/reports?status=queue&limit=120"),api("/api/admin/prompt-duplicates?limit=120"),api("/api/admin/prompt-audits?limit=160"),api("/api/admin/rum"),api("/api/admin/providers"),api("/api/gallery/leaderboard?range=week&limit=20"),api("/api/admin/gallery-like-anomalies?limit=80"),api("/api/admin/announcements?limit=200")]);adminState.version=t,adminState.settings=a,adminState.users=n.users||[],adminState.generations=r.records||[],adminState.prompts=s.prompts||[],adminState.promptSources=i.sources||[],adminState.promptSyncRuns=i.runs||[],adminState.tags=l.tags||[],adminState.promptCategories=d.categories||l.categories||[],adminState.tagSummary=l.summary||null,adminState.publicImages=c.generations||[],adminState.galleryFileChecks=u.checks||[],adminState.creditLedger=p.ledger||[],adminState.rewardLedger=b.rewards||[],adminState.audit=g.logs||adminState.audit,adminState.withdrawals=h.requests||[],adminState.reports=v.reports||[],adminState.promptDuplicates=o.candidates||[],adminState.promptAudits=m.audits||[],adminState.rum=y||{summary:{},events:[]},adminState.providers=w.providers||[],adminState.galleryLeaderboard=f.generations||[],adminState.galleryLikeAnomalies=S.anomalies||[],adminState.announcements=A.announcements||[],adminState.defaultProviderId=w.defaultProviderId||a.defaultProviderId||"",setStatus(`${adminState.user.name||adminState.user.email} · ${t.version}`,"ok")}function metrics(){const e=todayItems(adminState.generations),t=adminState.generations.length,a=adminState.generations.filter(i=>i.status==="success"||i.status==="succeeded").length,n=adminState.generations.filter(i=>i.status==="failed").length,r=adminState.generations.filter(i=>Number(i.durationMs)>0),s=r.reduce((i,l)=>i+Number(l.durationMs),0)/Math.max(1,r.length);return{todayGenerated:e.length,successRate:t?Math.round(a/t*100):0,failedRate:t?Math.round(n/t*100):0,avgDuration:s,newUsers:todayItems(adminState.users).length,publicWorks:adminState.publicImages.length,pendingReview:adminState.publicImages.filter(i=>i.isPublic).length,total:t}}function dashboardContext(){const e=adminState.galleryFileChecks.filter(o=>o.status==="broken"),t=(adminState.promptSyncRuns||[]).filter(o=>{const m=String(o.status||"").toLowerCase();return["failed","error","warning"].includes(m)}),a=(adminState.providers||[]).filter(o=>String(o.healthStatus||"").toLowerCase()==="error"),n=adminState.reports.length,r=adminState.withdrawals.filter(o=>String(o.withdrawalStatus||"")==="requested").length,s=adminState.rum?.summary||{},i=[],l=o=>i.push(o),d=[...adminState.reports].sort((o,m)=>new Date(m.createdAt||0)-new Date(o.createdAt||0))[0],c=[...e].sort((o,m)=>new Date(m.checkedAt||0)-new Date(o.checkedAt||0))[0],u=[...t].sort((o,m)=>new Date(m.startedAt||0)-new Date(o.startedAt||0))[0];c&&l({tone:"danger",icon:"ri-folder-warning-line",title:"文件异常",detail:`${c.generationId||"-"} · ${c.filename||c.relativePath||"未知文件"}`,meta:c.errorMessage||c.status||"broken",time:c.checkedAt,jump:"gallery-files"}),u&&l({tone:"warn",icon:"ri-sync-warning-line",title:"提示词同步失败",detail:`${u.sourceName||u.sourceId||"-"} · ${u.errorLog||u.message||u.status||"failed"}`,meta:`${fmtNumber(u.successCount||0)} 成功 / ${fmtNumber(u.failureCount||0)} 失败`,time:u.startedAt,jump:"prompt-cms"}),a.slice(0,2).forEach(o=>{l({tone:"danger",icon:"ri-plug-line",title:"供应商健康异常",detail:`${o.name||o.id||"-"} · ${o.lastError||o.healthStatus||"error"}`,meta:o.baseUrl||o.providerType||"",time:o.updatedAt||o.createdAt,jump:"providers"})}),n>0&&l({tone:"warn",icon:"ri-alarm-warning-line",title:"举报待处理",detail:`${fmtNumber(n)} 条公开作品举报正在队列中`,meta:d?`${d.id||"-"} · ${d.userName||d.userEmail||"匿名"}`:"等待人工审核",time:d?.createdAt||null,jump:"square-review"}),r>0&&l({tone:"warn",icon:"ri-inbox-unarchive-line",title:"撤回申请待处理",detail:`${fmtNumber(r)} 条公开撤回申请未处理`,meta:"需要在举报与撤回页处理",time:adminState.withdrawals.find(o=>String(o.withdrawalStatus||"")==="requested")?.withdrawalRequestedAt||null,jump:"reports-withdrawals"}),Number(s.imageFailures||0)>0&&l({tone:"warn",icon:"ri-image-warning-line",title:"图片失败事件",detail:`${fmtNumber(s.imageFailures)} 次图片加载或生成失败`,meta:`RUM 事件 ${fmtNumber(s.total||0)}`,time:s.updatedAt||null,jump:"rum-performance"});const p=e.length+t.length+a.length,b=n+r+Number(s.imageFailures||0),g=p>0?"danger":b>0?"warn":"ok",h=p>0?"需要关注":b>0?"有待处理项":"运行正常",v=p>0?`${fmtNumber(e.length)} 个文件异常 · ${fmtNumber(t.length)} 个同步失败 · ${fmtNumber(a.length)} 个供应商异常`:b>0?`${fmtNumber(n)} 条举报 · ${fmtNumber(r)} 条撤回 · ${fmtNumber(s.imageFailures||0)} 次图片失败`:"当前没有明显阻断项";return{tone:g,label:h,detail:v,reportQueue:n,brokenFiles:e,syncFailures:t,providerIssues:a,withdrawalQueue:r,rumSummary:s,issues:i.slice(0,6)}}function statCard(e,t,a,n,r="blue"){return`
    <article class="admin-stat" data-tone="${r}">
      <i class="${n}"></i>
      <span>${e}</span>
      <strong>${t}</strong>
      <small>${a}</small>
  ...
  `}function bindToolbar(){$("#adminSearchInput")?.addEventListener("input",e=>{adminState.search=e.target.value,adminState.page=1,render()}),$("#adminStatusFilter")?.addEventListener("change",e=>{adminState.status=e.target.value,adminState.page=1,render()})}function filtered(e,t){const a=adminState.search.trim().toLowerCase();return e.filter(n=>adminState.status==="all"||n.status===adminState.status||n.moderationStatus===adminState.status||n.withdrawalStatus===adminState.status||String(n.role||"")===adminState.status?a?t.some(s=>String(n[s]||"").toLowerCase().includes(a)):!0:!1)}function paged(e){const t=(adminState.page-1)*adminState.pageSize;return e.slice(t,t+adminState.pageSize)}function pagination(e){const t=Math.max(1,Math.ceil(e/adminState.pageSize));return`
  ...
  `}function emptyState(e,t,a="ri-inbox-archive-line",n="neutral"){return`
    <div class="admin-empty-state admin-empty-state-polished" data-tone="${escapeHtml(n)}">
      ...
      <div class="admin-table-wrap">
        <table class="admin-table">
          ...
          <tbody>
            ${t.map((n,r)=>`
              <tr>
                <td><strong>#${r+1} ${escapeHtml(n.id)}</strong><small class="admin-truncate">${escapeHtml(n.prompt||"")}</small></td>
  ...
  `),$("#drawerCreateUserForm").addEventListener("submit",async e=>{e.preventDefault();const t=new FormData(e.currentTarget),a=String(t.get("role")||"user");if(a==="admin"&&!await confirmAction({title:"创建管理员用户",message:"确认创建 admin 用户？该账号将拥有后台管理权限。",confirmText:"创建 admin",danger:!0}))return;const n=await api("/api/admin/users",{method:"POST",body:JSON.stringify({name:t.get("name"),email:t.get("email"),password:t.get("password"),generatePassword:!!t.get("generatePassword"),role:a,status:t.get("status"),credits:t.get("credits"),note:t.get("note")})});recordAudit("create_user",n.user?.id||String(t.get("email")),t.get("email")),await loadAll(),render(),n.temporaryPassword?showTemporaryPassword(n.temporaryPassword):closeDrawer()})}function providerDrawer(e={}){const t=!e.id,a={textToImage:!0,imageEdit:!0,imageToImage:!0,multiCandidate:!1,asyncTasks:!1,responses:!0,revisedPrompt:!0,sizes:[],qualities:[],formats:[],transparentBackground:!1,sourceTransparency:!1,privacyDownload:!1,maxImagesPerRequest:1,...e.capabilities||{}},n={role:t?"fallback":"default",weight:1,...e.routing||{}},r=e.mapping||{};openDrawer(t?"新增 Provider":"编辑 Provider",`
    <form id="drawerProviderForm" class="admin-form-grid single">
      ...
      <label>类型<select name="providerType">
        ${["openai","openai-compatible","custom-proxy"].map(s=>`<option value="${s}"${(e.providerType||"openai-compatible")===s?" selected":""}>${s}</option>`).join("")}
      </select></label>
      ...
      <label>Provider Mapping JSON<textarea name="mapping" rows="12" placeholder='{"mode":"openai-compatible","submit":{"method":"POST","path":"/v1/images/generations"}}'>${escapeHtml(JSON.stringify(r,null,2))}</textarea></label>
  ...
  `),$("#drawerProviderForm").addEventListener("submit",async s=>{s.preventDefault();const i=new FormData(s.currentTarget);let l,d,c;try{l=JSON.parse(i.get("capabilities")||"{}"),d=JSON.parse(i.get("routing")||"{}"),c=JSON.parse(i.get("mapping")||"{}")}catch{setStatus("Provider JSON 格式错误","danger");return}const u=String(i.get("apiKey")||"").trim(),p={name:i.get("name"),providerType:i.get("providerType"),baseUrl:i.get("baseUrl"),defaultModel:i.get("defaultModel"),endpointImages:i.get("endpointImages"),endpointResponses:i.get("endpointResponses"),endpointEdits:i.get("endpointEdits"),status:i.get("status"),sortOrder:i.get("sortOrder"),capabilities:l,routing:d,mapping:c};(t||u)&&(p.apiKey=u),await api(t?"/api/admin/providers":`/api/admin/providers/${encodeURIComponent(e.id)}`,{method:t?"POST":"PATCH",body:JSON.stringify(p)}),recordAudit(t?"create_provider":"update_provider",e.id||String(p.name),p.baseUrl),await refreshAndRender(),closeDrawer()}),$("[data-delete-provider]")?.addEventListener("click",async()=>{await confirmAction({title:"删除 Provider",message:`确认删除 ${e.name}？默认 Provider 不能删除。`,confirmText:"删除",danger:!0})&&(await api(`/api/admin/providers/${encodeURIComponent(e.id)}`,{method:"DELETE"}),recordAudit("delete_provider",e.id,e.name),await refreshAndRender(),closeDrawer())})}async function userDrawer(e){const[t,a,n]=await Promise.all([api(`/api/admin/users/${encodeURIComponent(e.id)}/credit-ledger?limit=80`),api(`/api/admin/users/${encodeURIComponent(e.id)}/reward-ledger?limit=80`),api(`/api/admin/users/${encodeURIComponent(e.id)}/generations?includeArchived=1&limit=80`)]),r=t.ledger||[],s=a.rewards||[],i=n.generations||[],l=e.firstPublicRewardStatus&&e.firstPublicRewardStatus!=="none"?`${e.firstPublicRewardStatus} · ${fmtNumber(e.firstPublicRewardAmount)} · ${e.firstPublicRewardGenerationId||"-"}`:"未发放";openDrawer("用户与积分",`
    ...
    <section class="admin-ledger-section">
      ...
      <div class="admin-mini-table">
        ${r.map(d=>`
          <div><strong>${d.delta>0?"+":""}${fmtNumber(d.delta)}</strong><span>${escapeHtml(d.source)}</span><small>${fmtNumber(d.balanceAfter)} · ${fmtDate(d.createdAt)}</small></div>
        `).join("")||"<p>暂无积分流水</p>"}
      </div>
    </section>
    <section class="admin-ledger-section">
      ...
      <div class="admin-mini-table">
        ${s.map(d=>`
          <div><strong>${escapeHtml(d.rewardType)}</strong><span>${escapeHtml(d.status)} · ${fmtNumber(d.amount)}</span><small>${fmtDate(d.awardedAt||d.createdAt)}</small></div>
        `).join("")||"<p>暂无奖励流水</p>"}
      </div>
    </section>
    <section class="admin-ledger-section">
  ...
  `),$("#drawerPromptSourceForm").addEventListener("submit",async a=>{a.preventDefault();let n;try{n=promptSourcePayload(new FormData(a.currentTarget))}catch(r){setStatus(r.message,"danger");return}await api(t?"/api/admin/prompt-sources":`/api/admin/prompt-sources/${encodeURIComponent(e.id)}`,{method:t?"POST":"PATCH",body:JSON.stringify(n)}),recordAudit(t?"create_prompt_source":"update_prompt_source",e.id||n.name,n.name),await refreshAndRender(),closeDrawer()})}function promptPayload(e){return{title:e.get("title"),image:e.get("imageUrl"),preview:e.get("preview"),prompt:e.get("prompt"),tags:String(e.get("tags")||"").split(/[,，\s]+/).filter(Boolean),category:e.get("category"),visibility:e.get("visibility"),author:e.get("author"),source:e.get("source"),sourceUrl:e.get("sourceUrl"),githubUrl:e.get("githubUrl"),remoteId:e.get("remoteId"),sourceRepo:e.get("sourceRepo"),sourceCategory:e.get("sourceCategory"),promptType:e.get("promptType"),language:e.get("language"),modelHint:e.get("modelHint"),syncedAt:e.get("syncedAt"),status:e.get("status"),sortOrder:Number(e.get("sortOrder")||0)}}function promptDrawer(e={}){const t=!e.id,a=(adminState.promptCategories||[]).filter(r=>r.status!=="hidden"),n=e.category||"general";openDrawer(t?"新建提示词":"编辑提示词",`
    <form id="drawerPromptForm" class="admin-form-grid single">
      ...
      <label>分类<select name="category">
        ${a.map(r=>`<option value="${escapeHtml(r.slug)}"${n===r.slug?" selected":""}>${escapeHtml(r.labelZh||r.slug)}</option>`).join("")}
        ${a.some(r=>r.slug===n)?"":`<option value="${escapeHtml(n)}" selected>${escapeHtml(n)}</option>`}
      </select></label>
  ...
  `),$("#drawerPromptForm").addEventListener("submit",async r=>{r.preventDefault();const s=promptPayload(new FormData(r.currentTarget));await api(t?"/api/prompts":`/api/prompts/${e.id}`,{method:t?"POST":"PATCH",body:JSON.stringify(s)}),recordAudit(t?"create_prompt":"update_prompt",String(e.id||s.title),s.title),await refreshAndRender(),closeDrawer()}),$("[data-hide-prompt]")?.addEventListener("click",async()=>{await confirmAction({title:"隐藏提示词",message:"确认隐藏该提示词？隐藏后前台将不再展示。",confirmText:"隐藏",danger:!0})&&(await api(`/api/prompts/${e.id}`,{method:"DELETE"}),recordAudit("hide_prompt",String(e.id),e.title||""),await refreshAndRender(),closeDrawer())})}function categoryDrawer(e={}){const t=!e.slug;openDrawer(t?"新建提示词分类":"编辑提示词分类",`
  ...
  `),$("#drawerCategoryForm").addEventListener("submit",async a=>{a.preventDefault();const n=new FormData(a.currentTarget);if(n.get("status")==="hidden"&&!await confirmAction({title:"停用分类",message:"确认停用该分类？前台筛选将不再展示停用分类名。",confirmText:"停用",danger:!0}))return;const r={slug:String(n.get("slug")||"").trim().toLowerCase(),labelZh:n.get("labelZh"),labelEn:n.get("labelEn"),descriptionZh:n.get("descriptionZh"),descriptionEn:n.get("descriptionEn"),status:n.get("status"),sortOrder:Number(n.get("sortOrder")||0)};await api(t?"/api/prompt-categories":`/api/prompt-categories/${encodeURIComponent(e.slug)}`,{method:t?"POST":"PATCH",body:JSON.stringify(r)}),recordAudit(t?"create_prompt_category":"update_prompt_category",r.slug,r.labelZh||""),await refreshAndRender(),closeDrawer()}),$("[data-hide-category]")?.addEventListener("click",async()=>{await confirmAction({title:"停用分类",message:"确认停用该分类？已有标签不会删除。",confirmText:"停用",danger:!0})&&(await api(`/api/prompt-categories/${encodeURIComponent(e.slug)}`,{method:"DELETE"}),recordAudit("hide_prompt_category",e.slug,e.labelZh||""),await refreshAndRender(),closeDrawer())})}function tagDrawer(e={}){const t=!e.slug;openDrawer(t?"新建标签":"编辑标签",`
  ...
  `),$("#drawerTagForm").addEventListener("submit",async a=>{a.preventDefault();const n=new FormData(a.currentTarget);if(n.get("status")==="hidden"&&!await confirmAction({title:"隐藏标签",message:"确认隐藏该标签？前台筛选和推荐可能受影响。",confirmText:"隐藏",danger:!0}))return;const r={slug:String(n.get("slug")||"").trim().toLowerCase(),labelZh:n.get("labelZh"),labelEn:n.get("labelEn"),aliases:String(n.get("aliases")||"").split(/[,，\n]+/).map(s=>s.trim()).filter(Boolean),category:n.get("category"),status:n.get("status"),sortOrder:Number(n.get("sortOrder")||0),hue:Number(n.get("hue")||0),showInFilter:!!n.get("showInFilter"),source:e.source||"admin"};await api(t?"/api/tags":`/api/tags/${encodeURIComponent(e.slug)}`,{method:t?"POST":"PATCH",body:JSON.stringify(r)}),recordAudit(t?"create_tag":"update_tag",r.slug,r.labelZh||r.labelEn||""),await refreshAndRender(),closeDrawer()}),$("[data-merge-tag]")?.addEventListener("click",async()=>{const a=$("#mergeTargetSlug").value.trim().toLowerCase();!a||!await confirmAction({title:"合并标签",message:`确认把 ${e.slug} 合并到 ${a}？历史内容会迁移到目标标签。`,confirmText:"合并",danger:!0})||(await api(`/api/tags/${encodeURIComponent(e.slug)}/merge`,{method:"POST",body:JSON.stringify({targetSlug:a})}),recordAudit("merge_tag",e.slug,`target=${a}`),await refreshAndRender(),closeDrawer())})}function workDrawer(e){openDrawer("公开作品详情",`
