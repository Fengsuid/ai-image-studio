...
This directory intentionally starts as a small JavaScript source baseline. It defines the source boundary, API adapter boundary, schema conversion boundary, and upstream licensing notes before the larger editor migration begins.

## Boundaries

- Source lives under `apps/canvas-v2/src`.
- Build output is generated into `public/canvas-v2` by `npm run build --prefix apps/canvas-v2`.
- Browser code must call this project's Node API through `src/adapters/ai-image-studio-api.js`.
