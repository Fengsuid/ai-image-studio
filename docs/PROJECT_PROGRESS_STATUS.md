...
| `AIS-RLS-094` - `AIS-RLS-105` | Done | Phase A documentation/baseline cleanup, visual baseline promotion, lint/check tooling foundation, first Phase B route extractions for credits, public settings/growth, public announcements/stats, image history/bulk operations, image generation/edit endpoints, and admin route business-domain split. |

## Backlog Milestones (Planned)

| Range | Status | Scope |
| --- | --- | --- |
| `AIS-RLS-106` - `AIS-RLS-132` | Ready / blocked by dependency | Follow-up Optimization Plan: Code Maintenance, Performance, Visual Polish, and Features. Newly added follow-ups: `AIS-RLS-130` ESLint tightening, `AIS-RLS-131` docs expansion, and `AIS-RLS-132` premium polish CSS split. |

## Active Task

| Task | Status | What Is Implemented | Remaining Closure |
| --- | --- | --- | --- |
| None | Done | Local Trellis board has no active tasks after `AIS-RLS-105`. `AIS-RLS-105` passed `npm run check`, `npm run smoke:admin-module-split`, `npm run smoke:server-route-boundary-split`, `npm run smoke:admin-generation-diagnostics`, `npm run smoke:public-reward-policy`, `npm run smoke:generation-trace`, `npm run smoke:mysql-store-domain-split`, container `npm run smoke:auth-admin -- http://127.0.0.1:3000`, online `npm run smoke:public -- https://<host>`, and online `npm run smoke:gallery-images -- https://<host>`. | Next ready item is `AIS-RLS-106` mysql-store facade programmatic rewrite. Do not proceed without user confirmation per 2026-05-25 handoff instruction. |
