...
> **2026-07-30 Quality Hardening Closeout** — `AIS-RLS-161` through `AIS-RLS-164`, plus `AIS-RLS-173` through `AIS-RLS-176`, are complete. Root Vitest collects Canvas Core tests in CI; the default check gate runs 25 deterministic smoke commands; Canvas v1 archival has an evidence-backed 30-day gate; gallery logic is entity-migrated from `app.js` to `app-gallery.js`; two stale production prompt thumbnails were remapped to verified local assets; and public/gallery-image online gates are green.
...
- Backlog tasks: 7 (`AIS-RLS-165` ~ `AIS-RLS-171`).
- Ready/executable tasks: 3 (`AIS-RLS-165`, `AIS-RLS-167`, `AIS-RLS-168`).
- Working tree handoff: the AIS-RLS-164 release commit is deployed with public/gallery-image gates green; local deployment archives are disposable and excluded from Git.
- Current blocker class: external acceptance evidence (`118`, `139`), the Canvas v1 archive time/regression gate (`159`), and dependency-gated backlog items (`166`, `169`, `170`, `171`).

## AIS-RLS-100~176 Status Snapshot

| Group | Board status | Count | Tasks |
| --- | --- | ---: | --- |
| Completed implementation / docs / QA tasks | Done | 67 | `AIS-RLS-100` ~ `AIS-RLS-117`, `AIS-RLS-119` ~ `AIS-RLS-138`, `AIS-RLS-140` ~ `AIS-RLS-158`, `AIS-RLS-160` ~ `AIS-RLS-164`, `AIS-RLS-172` ~ `AIS-RLS-176` |
| External/product evidence blockers | Blocked | 3 | `AIS-RLS-118`, `AIS-RLS-139`, `AIS-RLS-159` |
| Ready backlog | Backlog | 3 | `AIS-RLS-165`, `AIS-RLS-167`, `AIS-RLS-168` |
| Dependency-gated backlog | Backlog | 4 | `AIS-RLS-166`, `AIS-RLS-169`, `AIS-RLS-170`, `AIS-RLS-171` |
...
| `AIS-RLS-161` - `AIS-RLS-164`, `AIS-RLS-172` - `AIS-RLS-176` | Done | Canvas Core test collection, CI smoke expansion, Canvas v1 regression-window automation, gallery entity extraction, Agent/Canvas usability refresh, Agent plan confirmation integrity, planner timeout fallback, authenticated smoke ordering, and release-evidence closeout. |
...
| `AIS-RLS-165` - `AIS-RLS-171` | Backlog | app.js generation/session split, resumable generation, persistent rate limits, multi-candidate design, and external-evidence automation follow-ups. |

## Active Task

| Task | Status | What Is Implemented | Remaining Closure |
| --- | --- | --- | --- |
| None | Ready for next task | Gallery logic is entity-migrated into `app-gallery.js`; public and gallery-image online smoke pass against the deployed hashed assets. | Start `AIS-RLS-165`, the smallest ready backlog task, and continue serially. |
...
1. Start `AIS-RLS-165` and entity-migrate generation/session logic out of `public/app.js` while preserving behavior.
