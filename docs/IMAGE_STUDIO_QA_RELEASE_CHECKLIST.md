...
- Known blockers: full restore drill is intentionally deferred to a separate test host or local Docker/MySQL environment because the production host has `1.9Gi` RAM and no swap; slow-log probe `SELECT SLEEP(1.2)` was recorded with `Query_time: 1.207270`.
