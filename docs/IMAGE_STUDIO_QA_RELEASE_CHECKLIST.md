...
The audit covers all 92 `scripts/smoke/check-*.mjs` files. Helper modules `css-imports.mjs` and `frontend-manifest-helper.mjs` are not standalone smokes and are excluded from the count.

| Label | Count | Dependency rule | Default CI policy |
| --- | ---: | --- | --- |
| A — offline | 69 | Pure source/file/logic assertions or in-process route/store mocks; no live origin or database is required. | Eligible for `npm run check`; a failing assertion must fail CI and must never be converted to a credential-based skip. |
...
- **A — AIS-RLS-163 addendum (1):** `check-canvas-v2-regression-window.mjs`.
...
- AIS-RLS-163 adds the offline `smoke:canvas-v2-regression-window` gate to the same aggregate, bringing the current default total to 25 smoke commands.
...
- Known blockers: external `npm run smoke:visual-regression -- https://<host>` repeatedly navigated Chrome to `chrome-error://chromewebdata/` for every scenario, producing selector-missing diffs unrelated to deployed DOM/CSS; the same local visual regression passed before deployment, and external `smoke:mobile-layout` passed against the deployed site. At this release time, the three canvas hashed-entry smoke failures listed in §9.1 were known pre-existing `AIS-RLS-111` cleanup items, not AIS-RLS-114 regressions; this note is superseded by AIS-RLS-160.
...
- Known blockers: none for AIS-RLS-115 after local visual regression, container public smoke, and external public smoke passed. At this release time, the three hashed-entry canvas smoke failures listed in §9.1 were known pre-existing `AIS-RLS-111` cleanup items and were not part of this animation-library release gate; this note is superseded by AIS-RLS-160.
...
- Known blockers: no AIS-RLS-116 regressions found. At this release time, the three hashed-entry canvas smoke failures listed in §9.1 were known pre-existing `AIS-RLS-111` cleanup items and were not part of this skeleton release gate; this note is superseded by AIS-RLS-160.
...
- Known blockers: local MySQL credentials still block the four canvas API smokes that need DB-backed local startup, so those were covered in the deployed container instead. At this release time, `smoke:canvas-module-boundaries`, `smoke:canvas-layout-edges`, and `smoke:canvas-v2:entry` were known pre-existing `AIS-RLS-111` content-hash cleanup gaps, not slice extraction regressions; this note is superseded by AIS-RLS-160.
...
- Commit covered: `bd7342a feat(AIS-RLS-122): add my works asset library controls`.
- Files changed: `public/app-auth.js`, `public/app.js`, `public/app-settings.js`, `src/routes/images.js`, `scripts/smoke/check-my-works-asset-library.mjs`, `scripts/smoke/check-reference-assets.mjs`, `package.json`, My Works CSS modules, APP_VERSION/build manifests, `public/index.html`, and refreshed hashed `public/dist/*` assets.
- Acceptance coverage: My Works now exposes explicit asset-library controls for type/status tabs plus date and tag filters, preserves the large workspace/detail drawer, adds batch export and delete actions, and maps delete to the existing soft archive/unpublish policy instead of removing audit files.
- Backend coverage: `/api/images/history` accepts additive `status`, `type`, `tag`, `dateFrom`, `dateTo`, and `q` filters while preserving old defaults; `/api/images/bulk` now supports `export` manifest responses and `delete` soft-archive patches with the same ownership and public-withdrawal checks as archive/unpublish.
- Smoke coverage: added `smoke:my-works-asset-library` and included it in `npm run check`; the smoke verifies source + hashed dist UI hooks, CSS hooks, history filter behavior, bulk export manifest behavior, and delete soft-archive behavior.
- Local checks: `npm run frontend:build`, focused `node --check` for changed JS/smoke files, `npm run smoke:my-works-asset-library`, `npm run smoke:frontend-boundaries`, `npm run smoke:frontend-build-tooling`, `npm run check`, JSON parse check, and `git diff --check` passed. Local bare `npm run smoke:public` without a running local app failed with `localhost:3000` fetch setup; deployed/container public smoke below is the release authority.
- Deployment package: full tracked archive from `bd7342a`, `242,358,727` bytes / `1,857` entries / SHA256 `6A07A36BFEC95794956970E327C7E8323ECC07D67990456F90FDED3DF56CA7FE`; server-side SHA256 and entry count matched before extraction.
- Deployment note: production deployment clears stale `public/dist`, extracts the tracked archive, updates runtime to `APP_VERSION=20260531-my-works-asset-library-v1`, and rebuilds/restarts the app container. No database schema or data migration is included.
- Online smoke: container `npm run smoke:public -- http://<app-container>:3000`, container `npm run smoke:public -- https://<host>`, and container `npm run smoke:my-works-asset-library` passed; `/api/version` reports `20260531-my-works-asset-library-v1`. Local direct external smoke was blocked by the operator machine network path, while proxy `/api/version` and server-side HTTPS checks succeeded.
...
### 2026-06-01 AIS-RLS-125 Frontend Error Monitoring Release

- Task covered: `AIS-RLS-125` frontend JS error monitoring via `window.onerror` and `unhandledrejection`.
- Commit covered: final AIS-RLS-125 release commit; short hash is recorded in the task output and private deployment log because this public release record is part of the same single-task commit.
- Files changed: `public/client-error-monitor.js`, `public/app-router.js`, `src/routes/health.js`, `src/middleware/csrf.js`, `scripts/smoke/check-client-error-monitoring.mjs`, `scripts/smoke/check-maintenance-boundaries.mjs`, `package.json`, APP_VERSION/build manifests, `public/index.html`, `public/admin.html`, and refreshed hashed `public/dist/*` assets.
- Runtime coverage: the public app and `/admin` load the client error monitor before app/router scripts; `error` capture, `unhandledrejection`, and dynamically injected script failures report to `/api/client-error` with message, source, line, column, stack, user agent, route source, and session id.
- Backend coverage: `/api/client-error` is CSRF-exempt, records sanitized reports into the same in-memory RUM diagnostics buffer used by `/api/csp-report`, and emits `[client-error]` logs for server-side alert pickup.
- Dedupe coverage: the browser monitor suppresses identical same-session reports for `10s`; lazy route errors are also reported through `ImageStudioClientErrorMonitor.report`.
- Smoke coverage: added `smoke:client-error-monitoring` and included it in `npm run check`; the smoke verifies source plus hashed dist wiring, script order, CSRF exemption, CSP/client-error shared RUM channel, required report fields, lazy route reporting, and backend log emission.
- Local checks: focused `node --check` for changed server/frontend/smoke files, `npm run smoke:client-error-monitoring`, `npm run smoke:frontend-build-tooling`, `npm run smoke:frontend-boundaries`, `npm run smoke:maintenance-boundaries`, `npm run smoke:mysql-store-domain-split`, `npm run check`, and `git diff --check` passed. Local bare `npm run smoke:public` without a running app failed with `localhost:3000` fetch setup, and a temporary local app start was blocked by local MySQL credentials (`ER_ACCESS_DENIED_ERROR`); deployed/container public smoke below is the release authority.
- Deployment note: production deployment updates runtime to `APP_VERSION=20260601-client-error-monitoring-v1`; deployment clears stale `public/dist`, extracts the tracked archive, rebuilds/restarts the app container, and makes no database schema or data changes.
- Online smoke: container `npm run smoke:client-error-monitoring`, container `npm run smoke:public -- http://<app-container>:3000`, and external `npm run smoke:public -- https://<host>` passed; `/api/version` reports `20260601-client-error-monitoring-v1`; a direct POST to `/api/client-error` returned `204` and produced a `[client-error]` app log entry.
...
### 2026-06-01 AIS-RLS-132 Premium Polish Split Release

- Task covered: `AIS-RLS-132` split and land premium polish theme CSS safely.
- Commit covered: final AIS-RLS-132 release commit; short hash is recorded in the task output and private deployment log because this public release record is part of the same single-task commit.
- Files changed: new split CSS modules `public/css/14-premium-ambient.css`, `public/css/14-premium-interactions.css`, and `public/css/14-premium-mobile.css`; `public/styles.css`; self-hosted Remixicon compatibility CSS; six promoted visual-regression baselines; APP_VERSION/build manifest files; `public/index.html`; and refreshed hashed `public/dist/*` assets.
- Acceptance coverage: archived `archive/codex-handoff-20260524/14-premium-polish.css` was used as reference only, not imported or submitted as a production file. Premium visual work is split into three focused CSS modules of `58`, `69`, and `70` lines, each below the 500-line budget and with no new hard-coded hex literals.
- Runtime coverage: the release adds premium ambient mesh/glass surfaces, button/card hover polish, mobile bottom-nav/topbar density refinements, prompt card use-button overflow protection, and dark-mode readability protection for My Works detail title/body/meta/source panels.
- Build coverage: `npm run frontend:build` produced `/dist/app.01bc98b323f3.css`; `public/index.html`, `public/frontend-build-manifest.js`, and `public/frontend-build-manifest.json` all reference the new hashed CSS entry and list the three premium CSS sources.
- Visual QA: `npm run smoke:visual-regression` first reported six expected baseline diffs. Screenshots were manually reviewed for `home-composer-light-desktop`, `home-composer-dark-mobile`, `gallery-library-light-desktop`, `gallery-detail-modal-light-mobile`, `my-works-dark-mobile`, and `admin-shell-dark-mobile`; My Works dark mobile was tightened for readability before promotion. Only those six approved baselines were updated, then the full 10-scenario visual regression smoke passed with all baselines matched.
- Local checks: `node --check src/config/app-settings.js`, `node --check src/frontend/app-build-manifest.mjs`, `node --check public/frontend-build-manifest.js`, `npm run smoke:css-visual-polish`, `npm run smoke:frontend-boundaries`, `npm run smoke:visual-regression`, `npm run check`, and `git diff --check` passed. `npm run check` retained only existing long-term `public/app.js` and `public/styles.css` frontend-boundary warnings.
- Deployment note: deployed from the full release archive to the production container on the new ccs host, cleared stale `public/dist` before extraction, rebuilt/restarted the app image, and updated runtime to `APP_VERSION=20260601-premium-polish-split-v1`. No database schema or data migration is included.
- Online smoke: external `npm run smoke:public -- https://<host>` passed. Container one-shot `npm run smoke:css-visual-polish`, `npm run smoke:frontend-boundaries`, and `npm run smoke:public -- http://<app-container>:3000` passed. `/api/version` reported `20260601-premium-polish-split-v1`; app restart count was `0`, MySQL restart count was `0`, and MySQL remained healthy.
- Known blockers: none for AIS-RLS-132 after local visual baseline review.
- Rollback target: revert this release commit, restore `APP_VERSION=20260601-lazy-load-state-machine-v1` and prior hashed CSS entry `app.e2c89f60e518.css`, redeploy from git, then rerun public, frontend-boundaries, css-visual-polish, and visual-regression smoke.

### 2026-06-01 AIS-RLS-135 Public Topbar Density Release

- Task covered: `AIS-RLS-135` public topbar density governance.
- Commits covered: `34b7515 fix(AIS-RLS-135): compact public topbar density` and follow-up `9a9a819 fix(AIS-RLS-135): use self-hosted overflow icon`.
- Files changed: `public/index.html`, `public/css/05-home-shell.css`, `public/css/11-mobile-shell.css`, `public/css/06-gallery-leaderboard-responsive.css`, `public/app.js`, `public/app-auth.js`, `scripts/smoke/check-topbar-density.mjs`, `scripts/smoke/check-visual-regression.mjs`, `scripts/smoke/check-frontend-boundaries.mjs`, `scripts/smoke/check-mobile-layout.mjs`, `package.json`, APP_VERSION/build manifest files, refreshed hashed `public/dist/*` assets, and promoted home visual-regression baselines.
- Acceptance coverage: desktop topbar controls were reduced to the required compact set; tablet-width low-frequency actions move into the overflow menu; mobile keeps logo, search, and avatar while relying on bottom navigation/command entry; topbar height caps are guarded; legacy topbar pill classes are rejected; keyboard and accessibility attributes are preserved.
- Icon follow-up: the audit-discovered `ri-more-2-fill` regression was replaced with self-hosted `ri-more-2-line`, restoring `smoke:css-visual-polish` compatibility without adding another icon shim.
- Visual QA: `npm run smoke:visual-regression -- --filter home` produced the AIS-RLS-135 manual evidence run for `1440`, `1280`, `768`, and `375` viewport checks; the reviewed home baselines were promoted only for the intended topbar density deltas.
- Local checks: `npm run frontend:build`, `npm run smoke:topbar-density`, `npm run smoke:css-visual-polish`, `npm run smoke:visual-regression -- --filter home`, `npm run check`, and `git diff --check` passed. `npm run check` retained only existing long-term frontend-boundary warnings and the intentional lazy-load simulated failure stack.
- Deployment package: full tracked archive from `9a9a819`, `242,176,455` bytes / `1,869` entries / SHA256 `89B9E25FC6738FD7D48E760F80621126D0A759A734E97636E3A1C2807C7107AB`; server-side SHA256 and entry count matched before extraction.
- Deployment note: production deployment updates runtime to `APP_VERSION=20260601-topbar-density-v1`; deployment clears stale `public/dist`, extracts source plus generated assets together, rebuilds/restarts the app container, and makes no database schema or data changes.
- Online smoke: container `npm run smoke:css-visual-polish`, `npm run smoke:topbar-density`, and `npm run smoke:public -- http://<app-container>:3000` passed; external `npm run smoke:public -- https://<host>` passed; `/api/version` reports `20260601-topbar-density-v1`.
- Deployment health: app restart count is `0`, MySQL restart count is `0`, MySQL remains healthy, and the rebuilt app image manifest was recorded in the private deployment log.
- Known blockers: none for AIS-RLS-135 after the audit follow-up fix and redeploy.
- Rollback target: revert `9a9a819`, `34b7515`, and this release-record commit, restore `APP_VERSION=20260601-premium-polish-split-v1` with the previous topbar/dist entries, redeploy from git, then rerun public, css-visual-polish, topbar-density, and visual-regression smoke.

### 2026-06-01 AIS-RLS-135 P0 Topbar Entry Access Hotfix

- Incident covered: public home topbar core entries became unreachable after the compact topbar release because Canvas, Agent, and GitHub were moved into an overflow menu whose trigger was hidden for desktop and mobile ranges.
- Commits covered: `e1b91ee fix(AIS-RLS-135): restore core topbar entry access` and `eff55a4 test(AIS-RLS-135): harden topbar visual smoke`.
- Files changed: `public/index.html`, `public/css/05-home-shell.css`, `public/css/11-mobile-shell.css`, `scripts/smoke/check-topbar-density.mjs`, `scripts/smoke/check-visual-regression.mjs`, APP_VERSION/build manifest files, and refreshed hashed `public/dist/*` assets.
- Acceptance coverage: desktop and mobile now expose a visible `#topbarOverflowBtn` workspace trigger; the trigger has a visible desktop label and accessible name covering Canvas, Agent, and GitHub; the overflow menu marks `#canvasWorkspaceBtn`, `#agentWorkspaceBtn`, and `#topbarGithubLink` as protected core entries; mobile CSS preserves their touch targets.
- Smoke guardrail: `smoke:topbar-density` now rejects hidden core workspace access, and `smoke:visual-regression -- --filter topbar-density` clicks the workspace trigger and checks Canvas, Agent, and GitHub visibility across desktop, tablet, mobile, and dark desktop scenarios. The visual smoke was also hardened for anonymous production auth state and dynamic CDP ports.
- Local checks: `npm run frontend:build`, `npm run smoke:topbar-density`, `npm run smoke:css-visual-polish`, `npm run smoke:mobile-layout`, `npm run smoke:frontend-boundaries`, `npm run smoke:agent-workspace`, `npm run smoke:visual-regression -- --filter home`, `npm run smoke:visual-regression -- --filter topbar-density`, `npm run check`, and `git diff --check` passed. `npm run check` retained only existing frontend-boundary warnings and the intentional lazy-load simulated failure stack.
- Deployment package: full tracked archive from `eff55a4`, `242,177,933` bytes / `1,869` entries / SHA256 `C8EC1BD3D148FF12886905AF1C366F8983685897B16560D9D1BA667E68123001`; server-side SHA256 and entry count matched before extraction.
- Deployment note: production runtime remains `APP_VERSION=20260601-topbar-entry-hotfix-v1`; deployment clears stale `public/dist`, extracts source plus generated assets together, rebuilds/restarts the app container, and makes no database schema or data changes.
- Online smoke: container `npm run smoke:topbar-density`, container `npm run smoke:public -- http://<app-container>:3000`, external `npm run smoke:public -- https://<host>`, and external `npm run smoke:visual-regression -- https://<host> --filter topbar-density` passed; `/api/version` reports `20260601-topbar-entry-hotfix-v1`.
- Deployment health: app restart count is `0`, MySQL restart count is `0`, MySQL remains healthy, and the rebuilt app image manifest was recorded in the private deployment log.
- Known blockers: none for this P0 hotfix. The temporary visual/mobile smoke output directories remain local and ignored.
- Rollback target: revert `eff55a4` and `e1b91ee`, restore `APP_VERSION=20260601-topbar-density-v1` with the prior topbar/dist entries if needed, redeploy, then rerun public, css-visual-polish, topbar-density, and topbar visual-regression smoke.

### 2026-06-02 AIS-RLS-137 Motion Library Release

- Task covered: `AIS-RLS-137` token-driven public motion library, card spotlight, and reveal interactions.
- Commit covered: `44e8707 feat(AIS-RLS-137): add token-driven motion library`.
- Files changed: new `public/css/01-motion-library.css`, new `public/app-motion.js`, `public/app.js`, `public/css/12-animations.css`, `public/css/14-premium-interactions.css`, `public/styles.css`, `public/index.html`, visual smoke/module guard scripts, APP_VERSION/build manifest files, refreshed hashed `public/dist/*` assets, and approved gallery/leaderboard visual baselines.
- Acceptance coverage: the motion library defines `shimmer`, `fade-up`, `spring-in`, `pulse-soft`, and `floating-blob`; reusable motion utilities consume `var(--dur-*)` and `var(--ease-*)`; recent, prompt/gallery, history, and leaderboard cards are observed through `AppModules.motion`; spotlight coordinates use `--mx` / `--my`; reduced-motion mode disables entrance/spotlight animation and falls back to color-only hover.
- Smoke guardrail: `smoke:frontend-boundaries` now verifies the motion CSS module, token order, reduced-motion fallback, and stale premium spotlight coordinates. `smoke:public-app-module-split` verifies `app-motion.js` registration and that recent/history/library/leaderboard render paths re-observe newly inserted cards. `smoke:visual-regression -- --filter gallery,leaderboard` now includes a dedicated leaderboard page scenario.
- Local checks: `npm run frontend:build`, `node --check public\app.js`, `node --check public\app-motion.js`, `node --check scripts\smoke\check-frontend-boundaries.mjs`, `node --check scripts\smoke\check-public-app-module-split.mjs`, `node --check scripts\smoke\check-visual-regression.mjs`, `npm run smoke:frontend-boundaries`, `npm run smoke:public-app-module-split`, `npm run smoke:frontend-build-tooling`, `npm run check`, `npm run smoke:visual-regression -- --filter gallery,leaderboard`, and `git diff --check` passed. `npm run check` retained only the existing long-term frontend-boundary warnings and the intentional lazy-load simulated failure stack.
- Visual QA: gallery desktop, gallery dark mobile, gallery detail mobile, and leaderboard desktop screenshots were manually reviewed. The two gallery baselines were refreshed for the already-landed compact topbar state, and a new leaderboard baseline was promoted for this release.
- Deployment note: production runtime updates to `APP_VERSION=20260602-motion-library-v1`; deployment clears stale `public/dist`, extracts the tracked archive, rebuilds/restarts the app container, and makes no database schema or data changes.
- Online smoke: container `npm run smoke:public -- http://<app-container>:3000`, container `npm run smoke:frontend-boundaries`, container `npm run smoke:public-app-module-split`, and external `npm run smoke:public -- https://<host>` passed; `/api/version` reports `20260602-motion-library-v1`.
- Online visual note: external visual-regression navigation to `https://<host>` hit the known Chrome `chrome-error://chromewebdata/` failure mode. A tunnel run against the deployed container loaded real production pages and screenshots were manually reviewed; pixel diffs were expected because production data differs from the local mock baseline, not because of a layout regression.
- Deployment health: app restart count is `0`, MySQL restart count is `0`, MySQL remains healthy, and the rebuilt app image manifest is recorded in the private deployment log.
- Known blockers: none for AIS-RLS-137. Temporary visual-regression run directories are local ignored output and were intentionally left in place.
- Rollback target: revert `44e8707`, restore `APP_VERSION=20260601-topbar-entry-hotfix-v1` and the prior hashed entries, redeploy, then rerun public, frontend-boundaries, public-app-module-split, and gallery/leaderboard visual-regression smoke.

### 2026-06-02 AIS-RLS-139 Mobile Consolidation Release

- Task covered: `AIS-RLS-139` mobile CSS four-file consolidation, svh viewport cleanup, bottom navigation, and mobile bottom-sheet behavior.
- Commit covered: `f05ba72 fix(AIS-RLS-139): consolidate mobile nav and svh shell`.
- Files changed: mobile CSS consolidated into `public/css/mobile/_safe-area.css`, `_bottom-nav.css`, `_mobile-overrides.css`, and `_mobile-editor.css`; old mobile CSS modules were removed; `public/index.html`, `public/theme-mobile-nav.js`, `public/app.js`, APP_VERSION/build manifest files, hashed `public/dist/*` assets, smoke guardrails, frontend boundary docs, and two reviewed mobile visual baselines were updated.
- Acceptance coverage: `public/css` contains zero `100vh` matches; mobile bottom nav exposes `home / ranking / create / notifications / my` with the central create FAB raised by `top: -12px`; mobile toast is centered and docked above the safe-area bottom nav; legacy and primitive modals switch to bottom-sheet behavior at mobile widths with `align-items: end` and `max-height: calc(100svh - 56px)`; square preview and primitive split modals fold to `grid-template-columns: minmax(0, 1fr)`; `.primitive-modal--keep-centered` remains available for small centered dialogs.
- Smoke guardrail: `smoke:css-module-split` and `smoke:frontend-boundaries` now reject legacy mobile CSS files/imports, assert the new four mobile modules, assert no `100vh` in `public/css`, and verify bottom-nav, toast, bottom-sheet, and split-modal rules. `smoke:theme-mobile-nav`, `smoke:mobile-route-modal-behavior`, `smoke:topbar-density`, `smoke:agent-workspace`, and `smoke:visual-regression` were updated for the new mobile paths/actions.
- Local checks: `npm run frontend:build`, `npm run frontend:check`, `npm run smoke:css-module-split`, `npm run smoke:theme-mobile-nav`, `npm run smoke:mobile-route-modal-behavior`, `npm run smoke:frontend-boundaries`, `npm run smoke:topbar-density`, `npm run smoke:mobile-layout`, `npm run smoke:css-visual-polish`, `npm run smoke:agent-workspace`, `npm run check`, `npm run smoke:visual-regression`, `git diff --check`, and `rg -n '100vh' public/css -S` passed. `npm run check` retained only existing long-term frontend-boundary warnings and the intentional lazy-load simulated failure stack.
- Visual QA: `npm run smoke:visual-regression` first showed expected mobile baseline diffs for gallery detail and editor after the bottom-nav/topbar changes; both screenshots were manually reviewed for layout, reachable controls, and lack of overlap, then only those two baselines were promoted. The rerun passed all 15 scenarios.
- Deployment package: full tracked archive from `f05ba72`, `242,431,650` bytes / `1,869` entries / SHA256 `6F819D354514F7BD1ACCE4EAE50D63CA80EC90D3B27AF4FC657DB9B772E98515`; server-side SHA256 and entry count matched before extraction.
- Deployment note: production runtime updates to `APP_VERSION=20260602-mobile-consolidation-v1`; deployment extracts the tracked archive, removes stale deleted mobile CSS and old hashed dist assets from the server source tree, rebuilds/restarts the app container, and makes no database schema or data changes.
- Online smoke: container `npm run smoke:frontend-boundaries`, `npm run smoke:topbar-density`, `npm run smoke:theme-mobile-nav`, `npm run smoke:css-module-split`, and `npm run smoke:public -- http://<app-container>:3000` passed. External `npm run smoke:public -- https://<host>` passed and `/api/version` reports `20260602-mobile-consolidation-v1`.
- Online entry check: container-served HTML includes the new hashed CSS/JS entries, the desktop workspace trigger plus Canvas/Agent/GitHub menu items, and the five mobile bottom-nav actions. Container-served CSS includes the bottom nav FAB and mobile bottom-sheet rules.
- Known blockers: no automated blocker remains for AIS-RLS-139. The actual-device matrix still needs human device coverage on iOS/Android/iPad hardware; automated mobile viewport, visual, container, and external smoke passed.
- Rollback target: revert `f05ba72`, restore `APP_VERSION=20260602-motion-library-v1` and prior mobile CSS/dist entries, redeploy, then rerun public, frontend-boundaries, css-module-split, theme-mobile-nav, mobile-layout, and visual-regression smoke.

### 2026-06-02 AIS-RLS-143 Admin Cards Release

- Task covered: `AIS-RLS-143` Admin stat / hero / empty card primitive upgrade.
- Commit covered: this AIS-RLS-143 release commit; the final short hash is recorded in the task output and private deployment log.
- Files changed: new `public/css/primitives/_card.css`, `public/admin-overview.js`, admin shell/dashboard CSS refinements, `public/styles.css`, admin bootstrap cache-bust, APP_VERSION/build manifest files, refreshed hashed `public/dist/*` assets, updated admin primitive/lazy-load/visual smoke guards, and two approved admin visual baselines.
- Acceptance coverage: `.primitive-card`, `.primitive-card--stat`, `.primitive-card--hero`, and `.primitive-card--empty` are defined in the primitive layer; the admin dashboard hero consumes `.primitive-card--hero` and exposes four `.btn--secondary` quick actions; six dashboard stat cards consume `.primitive-card--stat` with `var(--fs-3xl)`, `var(--font-display)`, success/danger trend chips, and `data-sparkline-mount`; the no-issue dashboard state consumes `.primitive-card--empty` with inline SVG illustration, title, description, primary action, and helper link.
- Token coverage: the card primitive uses design tokens and `color-mix()` surfaces; `public/css/primitives/_card.css` has no hard-coded hex literals and stays under the 500-line CSS module budget.
- Smoke guardrail: `smoke:admin-primitives` now verifies the card primitive, import order, stat/hero/empty consumption, trend chips, sparkline mount slots, and `.btn--secondary` quick actions. `smoke:frontend-boundaries` now requires the card primitive CSS module. `smoke:lazy-load-state-machine` now validates admin cache-bust against the current frontend manifest version instead of a stale fixed version. `smoke:visual-regression -- --filter admin-dashboard` maps to the existing admin shell scenarios while asserting the new primitive card selectors.
- Local checks: `npm run frontend:build`, `node --check public\admin-overview.js`, `node --check scripts\smoke\check-admin-primitives.mjs`, `node --check scripts\smoke\check-frontend-boundaries.mjs`, `node --check scripts\smoke\check-lazy-load-state-machine.mjs`, `node --check scripts\smoke\check-visual-regression.mjs`, `npm run smoke:admin-primitives`, `npm run smoke:admin-module-split`, `npm run smoke:frontend-boundaries`, `npm run check`, `npm run smoke:visual-regression -- --filter admin-dashboard`, and `git diff --check` passed. `npm run check` retained only existing frontend-boundary warnings and the intentional lazy-load simulated failure stack.
- Visual QA: `npm run smoke:visual-regression -- --filter admin-dashboard` first produced expected diffs for `admin-shell-light-desktop` and `admin-shell-dark-mobile` after the hero/stat/card upgrade. Screenshots were manually reviewed for real dashboard load, visible quick actions, stat cards, mobile layout, and no obvious horizontal overflow; only those two admin baselines were promoted, and the rerun matched both baselines.
- Deployment note: production runtime updates to `APP_VERSION=20260602-admin-cards-v1`; deployment uses tracked archive extraction and app rebuild/restart, with no database schema or data migration.
- Online smoke: container `npm run smoke:admin-primitives`, `npm run smoke:admin-module-split`, `npm run smoke:frontend-boundaries`, and `npm run smoke:public -- http://127.0.0.1:3000` passed. External `npm run smoke:public -- https://<host>` passed, and container/external `/api/version` report `20260602-admin-cards-v1`.
- Known blockers: none for AIS-RLS-143 local acceptance after admin visual baseline review.
- Rollback target: revert this release commit, restore `APP_VERSION=20260602-mobile-consolidation-v1` and prior hashed admin/CSS entries, redeploy, then rerun public, admin-primitives, admin-module-split, and admin-dashboard visual smoke.

### 2026-06-02 AIS-RLS-144 Admin Interactions Release

- Task covered: `AIS-RLS-144` Admin micro-interactions and command palette.
- Commit covered: this AIS-RLS-144 release commit; the final short hash is recorded in the task output and private deployment log.
- Files changed: `public/admin/command-palette.js`, `public/admin/dashboard.js`, `public/admin.html`, `public/css/09-admin-shell-polish.css`, `scripts/smoke/check-admin-module-split.mjs`, APP_VERSION/build manifest files, `public/index.html`, and refreshed hashed `public/dist/*` admin/CSS assets.
- Acceptance coverage: admin table row hover now uses the required `color-mix(in srgb, var(--brand-50) 50%, transparent)` background plus `inset 3px 0 var(--brand-600)` brand inset; `/admin` exposes a visible `.btn--icon` command palette button in the topbar.
- Command palette coverage: `public/admin/command-palette.js` registers only under `[data-app="admin"]`, stays at 110 lines, uses `.primitive-modal--wide` with `data-flavor="palette"`, renders search, listbox commands, shortcut footer hints, substring matching, route commands, and localStorage-backed recent `user` / `order` / `prompt` entities.
- Keyboard coverage: desktop admin `Ctrl+K` / `Cmd+K` opens the palette; mobile skips the global shortcut but keeps the visible topbar button; Up/Down, Enter, Esc, and `/` are handled in the open palette. Public paths receive only a no-op `AdminCommandPalette` and do not register the global shortcut.
- Smoke guardrail: `smoke:admin-module-split` now verifies AIS-RLS-144 palette scope, 150-line budget, modal/listbox/search/footer hooks, keyboard tokens, recent localStorage keys, mobile shortcut guard, topbar button, dashboard recent persistence, and the required table hover / palette CSS selectors.
- Local checks: `npm run frontend:build`, focused `node --check` for changed admin JS and smoke files, `npm run smoke:admin-module-split`, `npm run smoke:admin-shell-polish`, `npm run check`, `git diff --check`, `wc -l public/admin/command-palette.js`, and a manual VM shortcut simulation passed. `npm run check` retained only existing frontend-boundary warnings and the intentional lazy-load simulated failure stack.
- Deployment package: full tracked archive from the initial AIS-RLS-144 runtime commit, `242,486,894` bytes / `1,870` entries / SHA256 `0F3733FCE400F3A2D7BDCAEE3DFA83B91A8CBF59E03CD43ED713C0C47EB0A123`; server-side SHA256 and entry count matched before extraction.
- Deployment note: production runtime updates to `APP_VERSION=20260602-admin-interactions-v1`; deployment extracts the tracked archive, removes the three stale admin/CSS hashed dist entries replaced by this release, rebuilds/restarts the app container, and makes no database schema or data changes.
- Online smoke: container `npm run smoke:admin-module-split`, `npm run smoke:admin-shell-polish`, `npm run smoke:frontend-boundaries`, and `npm run smoke:public -- http://127.0.0.1:3000` passed. External `npm run smoke:public -- https://<host>` passed, and container/external `/api/version` report `20260602-admin-interactions-v1`.
- Deployment health: app restart count is `0`, MySQL restart count is `0`, MySQL remains healthy, and stale hashed admin/CSS entries are absent while the new admin palette/dashboard/CSS hashes are present.
- Known blockers: none for AIS-RLS-144.
- Rollback target: revert this release commit, restore `APP_VERSION=20260602-admin-cards-v1` and prior hashed admin/CSS entries, redeploy, then rerun public, admin-module-split, admin-shell-polish, and frontend-boundaries smoke.

### 2026-06-02 AIS-RLS-146 Primitive CSS Architecture Release

- Task covered: `AIS-RLS-146` primitive CSS architecture finale.
- Commit covered: this AIS-RLS-146 release commit; the runtime deployment was first built from `a4e953e` before this release-record amend.
- Files changed: CSS architecture moved to `public/css/tokens.css`, `public/css/primitives/_*.css`, `public/css/pages/*.css`, and `public/css/mobile/_*.css`; old root page CSS modules were renamed into the page/mobile/primitive layers; new `_form`, `_pill`, and `_toast` primitives were added; `styles.css`, frontend build tooling, smoke guardrails, APP_VERSION/build manifests, hashed CSS bundle, and four reviewed visual baselines were updated.
- Acceptance coverage: `styles.css` now loads tokens/theme before primitives, pages, then mobile; primitive modules include `_button`, `_modal`, `_drawer`, `_toast`, `_table`, `_form`, `_card`, `_pill`, `_motion`, and `_motion-utilities`; no `04-components*` or `12-animations*` CSS modules remain under `public/css`; every page CSS module is within the 600-line budget; consumer layers have zero hard-coded hex after the legacy color alias bridge.
- Smoke guardrail: new `smoke:css-primitive-boundaries` verifies primitive/page/mobile directories, import order, exact `styles.css` module coverage, page files not defining `.btn` or `.primitive-*`, primitive files not carrying page-scope selectors, no old `04-components*` / `12-animations*` modules, and hard-coded hex staying within the task budget. `smoke:frontend-boundaries`, CSS split/build/performance/admin/mobile guardrails were updated for the new paths.
- Build coverage: the CSS bundle now minifies module contents while retaining source-section comments, keeping the initial hashed CSS bundle at `295075` bytes under the existing `320000` byte performance budget.
- Local checks: `npm run frontend:build`, `npm run frontend:check`, `npm run smoke:css-module-split`, `npm run smoke:css-primitive-boundaries`, `npm run smoke:frontend-build-tooling`, `npm run smoke:frontend-boundaries`, `npm run smoke:css-visual-polish`, `npm run smoke:frontend-performance`, `npm run smoke:admin-module-split`, `npm run smoke:admin-primitives`, `npm run smoke:admin-shell-polish`, `npm run smoke:mobile-route-modal-behavior`, `npm run smoke:theme-mobile-nav`, `npm run check`, `npm run smoke:visual-regression`, and `git diff --check` passed. `npm run check` retained only the existing frontend-boundary warnings and the intentional lazy-load simulated failure stack.
- Visual QA: four stale local baselines (`home-composer-light-desktop`, `home-composer-dark-mobile`, `gallery-detail-modal-light-mobile`, `admin-shell-dark-mobile`) were manually reviewed after topbar/mobile/admin changes, promoted, and the full 15-scenario local visual-regression rerun passed.
- Deployment package: full tracked archive from `a4e953e`, `242,492,860` bytes / `1,876` entries / SHA256 `5A148629F15D7F580DBD43A394C31D427A0687346DAD1391CE95CEAF96C0CEF1`; server-side SHA256 matched before extraction.
- Deployment note: production runtime updates to `APP_VERSION=20260602-primitive-architecture-v1`; deployment removed stale server-source `public/css` and `public/dist` before extracting the tracked archive, rebuilt/restarted the app container, and made no database schema or data changes.
- Online smoke: container `npm run smoke:css-primitive-boundaries` passed. External `npm run smoke:public -- https://<host>` passed, loading `/dist/app.d88ff5c573aa.css` and reporting `/api/version=20260602-primitive-architecture-v1`. External `npm run smoke:visual-regression -- https://<host> --filter topbar-density` passed for the four manual-evidence topbar scenarios. Full external visual-regression was not used as a pixel baseline because production data differs from the local mock baseline and produced expected content-driven diffs.
- Known blockers: none for AIS-RLS-146 after deployment. Temporary local smoke and visual-regression output directories were intentionally left in place.
- Rollback target: revert this AIS-RLS-146 release commit, restore `APP_VERSION=20260602-admin-interactions-v1` and prior root CSS/dist entries, redeploy, then rerun public, css-module-split, css-primitive-boundaries, frontend-boundaries, frontend-build-tooling, frontend-performance, and visual-regression smoke.

### 2026-06-02 AIS-RLS-160 Hashed Entry Smoke Migration

- Task covered: `AIS-RLS-160` migrate canvas smoke hard-coded entries to the frontend build manifest.
- Commit covered: this AIS-RLS-160 release commit.
- Files changed: shared `scripts/smoke/frontend-manifest-helper.mjs`, `check-canvas-module-boundaries.mjs`, `check-canvas-layout-edges.mjs`, `check-canvas-v2-entry.mjs`, and documentation updates for the former AIS-RLS-111 known smoke gap.
- Acceptance coverage: the three target smokes now resolve JS assets through `public/frontend-build-manifest.json`; canvas lazy route checks validate manifest-backed hashed `/dist/*.js` entries before reading the corresponding public source; `canvas-v2:entry` verifies `public/index.html` uses manifest-matched hashed `app`, `app-auth`, and generation-result-actions entries instead of `app.js?v=` cache busting.
- Local checks: `node --check` for the changed smoke/helper files, `npm run smoke:canvas-module-boundaries`, `npm run smoke:canvas-layout-edges`, `npm run smoke:canvas-v2:entry`, and `npm run smoke:frontend-build-tooling` passed before the full validation pass.
...
### 2026-06-02 AIS-RLS-136 Visual Hero Composer Release

- Task covered: `AIS-RLS-136` hero / composer glass-card upgrade with Instrument Serif display typography.
- Commit covered: this AIS-RLS-136 release commit; the final short hash is recorded in the task output and private deployment log.
- Files changed: `public/index.html`, `public/css/pages/home-shell.css`, `public/css/pages/visual-polish.css`, `public/css/pages/home-onboarding.css`, mobile hero spacing CSS, `public/css/primitives/_card.css`, visual-polish and visual-regression smoke guardrails, APP_VERSION/build manifest files, refreshed hashed CSS bundle, and two reviewed home composer baselines.
- Acceptance coverage: the hero title now uses `var(--font-display)` and a `brand-700 -> brand-500 -> #8b5cf6` clipped text gradient; the subtitle uses `var(--font-body)`, `var(--neutral-500)`, and `line-height: 1.5`; `#heroComposerMount` consumes `.primitive-card--hero` with `surface-glass`, `blur(20px) saturate(140%)`, `var(--shadow-lg)`, and a translucent 1px border; focused composer input transitions the glass-card border to `var(--brand-400)` over `var(--dur-base)`.
- Hero background coverage: two absolute-positioned radial-gradient blobs sit outside the hero flex flow, animate with the shared `floating-blob` keyframe over `30s`, stop under `prefers-reduced-motion: reduce`, and switch to `color-mix(in srgb, var(--brand-400) 40%, transparent)` in dark mode.
- Smoke guardrail: `smoke:css-visual-polish` now asserts the AIS-RLS-136 hero/composer tokens, blob DOM hooks, reduced-motion protection, CSP source split, and the `.hero > .hero-blob { position: absolute; }` regression guard. `smoke:visual-regression -- --filter home` now requires the primitive hero composer card plus both blob layers to be visible in the viewport.
- Local checks: `npm run frontend:build`, `node --check scripts\smoke\check-css-visual-polish.mjs`, `node --check scripts\smoke\check-visual-regression.mjs`, `npm run smoke:css-visual-polish`, `npm run smoke:visual-regression -- --filter home`, `npm run smoke:css-primitive-boundaries`, `npm run smoke:admin-primitives`, `npm run check`, and `git diff --check` passed. `npm run check` retained only the existing frontend-boundary line-count warnings and the intentional lazy-load simulated failure stack.
- Visual QA: `npm run smoke:visual-regression -- --filter home` produced run `2026-06-02T14-09-57-368Z`; `home-composer-light-desktop` and `home-composer-dark-mobile` matched reviewed baselines, and the four topbar-density home scenarios remained manual evidence. The reduced-motion check verified both blobs were static and absolute under `prefers-reduced-motion: reduce`.
- Deployment note: production runtime updates to `APP_VERSION=20260602-visual-hero-composer-v1`; deployment extracts the tracked archive from the AIS-RLS-136 runtime commit, clears stale `public/dist`, rebuilds/restarts the app container, and makes no database schema or data changes.
- Online smoke: container `npm run smoke:css-visual-polish`, `npm run smoke:topbar-density`, `npm run smoke:css-primitive-boundaries`, and `npm run smoke:public -- http://127.0.0.1:3000` passed. External `npm run smoke:public -- https://<host>` passed, loading `/dist/app.95bfd7dbde6f.css`, and external `/api/version` reports `20260602-visual-hero-composer-v1`.
- Online visual QA: external `npm run smoke:visual-regression -- https://<host> --filter home` passed the four topbar manual-evidence scenarios but produced expected pixel diffs for the two composer baseline scenarios because production login/API/reference-image state differs from the local reviewed baseline. Screenshots were manually reviewed; hero/composer layout, mobile spacing, topbar access, and generated hashed CSS were correct, so the production pixel diff was recorded as content-driven evidence rather than a release blocker.
- Known blockers: none after local acceptance, production public smoke, and manual review of the production visual evidence. Temporary local visual-regression run output is intentionally left in place per operator instruction.
- Rollback target: revert this AIS-RLS-136 release commit, restore `APP_VERSION=20260602-csp-enforce-v1` and prior hashed CSS entry `app.d88ff5c573aa.css`, redeploy, then rerun public, css-visual-polish, css-primitive-boundaries, topbar-density, and home visual-regression smoke.

### 2026-06-18 Admin Managed User Creation Hotfix

- Commit covered: `1e8b2e4 fix(admin): restore managed user creation`; the preceding `73d56b6` commit syncs the already-deployed AIS-RLS-136 runtime back to GitHub before this hotfix.
- Files changed: `public/admin/users.js`, `public/admin.html`, `public/frontend-build-manifest.*`, `public/dist/admin-users.663d95613e3e.js`, `scripts/smoke/check-admin-module-split.mjs`, and APP_VERSION/build manifest config.
- Fix coverage: the admin "新建用户" button now opens a dedicated create drawer instead of reusing the edit drawer with `id=""`; the create form collects email/name/role/status/credits and password mode, calls `POST /api/admin/users`, and shows the one-time temporary password returned by the backend.
- Regression guardrail: `smoke:admin-module-split` now asserts the create drawer, `POST /api/admin/users`, generate-password handling, and absence of the former empty-id `userDrawer` path.
- Local checks: `npm run frontend:build`, `node --check public\admin\users.js`, `node --check scripts\smoke\check-admin-module-split.mjs`, `npm run smoke:admin-module-split`, `npm run smoke:frontend-build-tooling`, `npm run check`, and `git diff --check` passed. `npm run smoke:auth-admin` was not runnable locally because `ADMIN_EMAIL` and `ADMIN_PASSWORD` were not provided.
- Deployment note: production runtime updates to `APP_VERSION=20260618-admin-user-create-v1`; deployment extracts the tracked archive, clears stale `public/dist`, rebuilds/restarts the app container, and makes no database schema or data migration.
- Online smoke: external `/api/version` reports `20260618-admin-user-create-v1`; deployed `/admin` uses the same cache-bust version; deployed `frontend-build-manifest.json` points `/admin/users.js` to `/dist/admin-users.663d95613e3e.js`; external `npm run smoke:public -- https://ai-image-studio.twisterfeng.com` passed and loaded the new admin users bundle.
- Rollback target: revert `1e8b2e4`, restore `APP_VERSION=20260602-visual-hero-composer-v1` and prior admin-users hash `admin-users.f826c642f180.js`, redeploy, then rerun admin-module-split and public smoke.

### 2026-06-19 Prompt Library Memory Hotfix

- Commits covered: `6d0d397 fix: reduce prompt library memory pressure` and follow-up `ed26895 test: fix prompt pagination smoke assertion`.
- Files changed: `/api/prompts` route/store pagination, `public/app.js` prompt-library paging, `public/cache-db.js` cached-image object URL cleanup, `public/admin/dashboard.js` bounded admin prompt startup load, APP_VERSION/build manifest files, refreshed hashed `public/dist/*` assets, and smoke guardrails.
- Fix coverage: prompt library initial load now requests `limit=120&offset=0` instead of `limit=2000`, tracks `pagination.hasMore`, and fetches subsequent pages only from the "加载更多" flow. `/api/prompts` now returns `{ pagination: { limit, offset, count, hasMore } }` and supports offset after display-level de-duplication.
- Memory cleanup coverage: cached gallery image blob URLs are revoked before replacement, on image failure/fallback paths, on DOM removal through `MutationObserver`, and on `pagehide`.
- Admin coverage: `/admin` initial prompt CMS request is bounded to 500 prompts instead of 2000, while other admin moderation/scan endpoints are unchanged.
- Local checks: `npm run frontend:build`, `npm run check`, `npm run smoke:frontend-performance`, `npm run smoke:prompt-library-polish`, `npm run smoke:indexeddb-cache-static`, `npm run smoke:gallery-experience-task46`, `node --check` for changed route/store/frontend/cache/smoke files, and external `npm run smoke:public -- https://ai-image-studio.twisterfeng.com` passed.
- Deployment package: full tracked archive from `6d0d397`, `242,544,426` bytes / SHA256 `B53E97EDA5B45C8287BBFC1B05F4E269324A7C4A122D4AF933686156F6C64EC4`; the final test-only `ed26895` script fix was synced to the server and the app image was rebuilt.
- Deployment note: production runtime updates to `APP_VERSION=20260619-memory-prompt-paging-v1`; deployment clears stale `public/dist`, extracts the tracked archive, updates `.env`, rebuilds/restarts the app container, then rebuilds once more after syncing the smoke script fix. No database schema or data migration.
- Online smoke: container and external `/api/version` report `20260619-memory-prompt-paging-v1`; external `/api/prompts?limit=3&offset=1` returns `pagination.hasMore=true`; external `npm run smoke:public -- https://ai-image-studio.twisterfeng.com` passed and loaded `/dist/app.792e3b86b56e.js`, `/dist/cache-db.cd1be9d848eb.js`, and `/dist/admin-dashboard.6282398ca294.js`.
- Rollback target: revert `6d0d397` and `ed26895`, restore `APP_VERSION=20260618-admin-user-create-v1` and prior app/cache/admin-dashboard hashed entries, redeploy, then rerun public, prompt-library-polish, indexeddb-cache-static, and frontend-performance smoke.

### 2026-06-19 AIS-RLS-138 Visual Dark Mode Release

- Task covered: `AIS-RLS-138` dark mode completion and ambient drift.
- Commit covered: this AIS-RLS-138 release commit; runtime archive was built from pre-release-record amend `b8d12e7`.
- Files changed: dark theme tokens, public/admin early theme bootstrap, public/admin theme persistence sync, admin sidebar desktop state normalization, CSP hash allowance, visual-regression dark scenarios, refreshed hashed frontend assets, and reviewed dark baselines.
- Acceptance coverage: dark `neutral/surface/text/border/shadow` mappings now render surfaces through dark tokens; hero/composer glass uses `color-mix(in srgb, var(--neutral-800) 78%, transparent)`; ambient/hero blobs use brighter `brand-400` dark treatment; `.theme-toggle` persists `imageStudio.theme` with prefers-color-scheme fallback; public and admin pages set `document.documentElement[data-theme]` in `<head>` before CSS loads.
- Visual QA: local `npm run smoke:visual-regression` passed 21 scenarios after promoting reviewed dark baselines for home, gallery/library, prompt detail, leaderboard, editor, my works, chat workspace, and admin shell including dark 1440.
- Local checks: `npm run frontend:build`, changed JS `node --check`, `npm run check`, `npm run smoke:visual-regression`, `npm run smoke:theme-mobile-nav`, `npm run smoke:csp-enforce`, `npm run smoke:frontend-build-tooling`, `npm run smoke:reference-assets`, and `git diff --check` passed. Local `npm run smoke:public` without a running local app failed with `fetch failed`; deployed external smoke below is the release authority.
- Deployment package: tracked `git archive` from `b8d12e7`, `246,338,142` bytes / `1,885` entries / SHA256 `AC4C6120EDF42F0A220CA543C92C7607072126F192994E5E17814F9102F4F197`.
- Deployment note: production runtime updates to `APP_VERSION=20260619-visual-dark-mode-v1`; deployment clears stale `public/dist`, extracts the tracked archive, rebuilds/restarts the app container, and makes no database schema or data migration.
- Online smoke: external `/api/version` reports `20260619-visual-dark-mode-v1`; external `frontend-build-manifest.json` reports `20260619-visual-dark-mode-v1`, `/dist/app.f5ba5a12417a.css`, `/dist/theme-mobile-nav.6b11e31ff9d2.js`, and `/dist/admin-dashboard.4499832d9341.js`; external `npm run smoke:public -- https://<host>` passed.
...
### 2026-07-28 AIS-RLS-161 Canvas Core Vitest Collection Release

- Task covered: `AIS-RLS-161` closes the Vitest include gap that left `packages/canvas-core/tests/**/*.test.js` outside the root test command and CI.
- Commit covered: `3df3867`.
- Fix coverage: `vitest.config.mjs` explicitly collects Canvas Core tests without skip, wrapper, or test-file relocation.
- Local checks: the task commit increased the root suite from 3 files / 14 tests to 6 files / 77 tests, with all collected tests passing; `npm run check` also passed.
- CI evidence: GitHub Actions run `30332926703`, job `90191720337`, executed `packages/canvas-core/tests/service.test.js` (23), `import-export.test.js` (26), and `references.test.js` (14). The later combined suite completed with 7/7 files and 81/81 tests passed.
- Deployment note: no server deployment was required because this task only changes test collection.
- Known blockers: none.
- Rollback target: revert `3df3867` to restore the previous include list.
...
- Online smoke: external public smoke passed. Authenticated planner-flow, batch-generation, workspace, credit-per-step, export-canvas, and batch-export all passed on the final AIS-RLS-175 runtime.
- Known blockers: none; production confirmation enforcement and all affected authenticated smoke flows are closed.
...
- Online smoke: external public smoke passed and authenticated Agent planner-flow completed successfully without the former client AbortError. Authenticated batch-generation and workspace also passed; the final AIS-RLS-175 runtime retained the same result.
- Known blockers: none.
...
- Deployment note: deployed as a verified Git delta over the AIS-RLS-174 runtime. Production reports `APP_VERSION=20260728-agent-smoke-confirm-v1`; no database schema or data migration changes are included.
...
### 2026-07-30 AIS-RLS-163 Canvas v2 Regression Window Gate Release

- Task covered: `AIS-RLS-163` replaces the manual Canvas v1 archive waiting judgment with a local, evidence-backed 30-day gate.
- Commit covered: the single AIS-RLS-163 release commit containing this record; its final short hash and GitHub Actions run are recorded in the Trellis closeout output.
- Evidence model: `docs/canvas-v2-regression-window.json` records the production baseline, required smoke set, timestamped results, and unresolved regressions. The baseline is `2026-07-26`, when AIS-RLS-154 first reached production through the AIS-RLS-172 release; local completion before deployment does not start the production window.
- Gate behavior: the default command reports baseline/deadline, days remaining, open blocking regressions, last full-green time, and a stable decision. `--require-ready` exits non-zero until the decision is `READY_FOR_AIS_RLS_159`; `--as-of` makes future-date decisions reproducible.
- Current result: as of `2026-07-30`, deadline `2026-08-25`, 26 days remain, blocking regressions are 0, and the last full-green run is `2026-07-30T12:00:00+08:00`; AIS-RLS-159 remains blocked by time only.
