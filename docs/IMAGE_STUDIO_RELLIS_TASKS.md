...
### AIS-RLS-046：画廊展示与输入体验综合修复

优先级：P0
标签：`frontend`, `gallery`, `prompt`, `image-editor`, `data-sync`, `qa`
依赖：`AIS-RLS-041`, `AIS-RLS-043`, `AIS-RLS-044`, `AIS-RLS-045`
建议状态：Backlog

目标：

- 修复最新回归反馈中的画廊图片展示、榜单布局、公开标题、对话管理、生成闪烁、admin 用户对话查看边界、输入区尺寸、图生图导入和新提示词源接入问题，确保画廊页面可以稳定展示图片和提示词数据。

交付物：

- 修复画廊里部分图片只显示灰色占位或渐变占位的问题；对远程提示词封面、GitHub raw 图片、不可访问图片增加可验证的 fallback、重试或代理策略。
- 将点赞榜单从当前画廊侧栏移出，改为独立榜单页面或可跳转的独立路由；画廊主页面保留明确入口，榜单页面完整承载日榜、周榜、月榜、总榜、点赞和详情跳转。
- 用户公开图片到画廊时新增“图片标题”选项；未填写标题时默认使用提示词前若干字符生成标题，并在画廊卡片、详情、榜单和管理后台保持一致。
- 对话列表增加删除对话功能，删除前二次确认；删除后不影响已保存的公开作品、我的作品或审计记录。
- 对话列表增加编辑对话标题功能，标题保存在会话元数据中，刷新后仍保持。
- 修复文生图生成轮询期间整段对话重建导致的闪烁，只更新生成中条目的进度状态。
- 关闭 admin 端普通个人历史里直接看到所有用户对话/作品的行为；需要查看时移动到后台用户管理，通过明确按钮查看指定用户对话列表。
- 收窄对话列表下方提示词输入框宽度，使其在桌面端不铺满过宽区域，移动端仍保持可用。
- 图生图上传区域支持用户粘贴图片和拖拽导入图片，和文件选择共用同一校验、预览、引用图状态与错误提示。
- 接入 `https://github.com/davidwuw0811-boop/awesome-gpt-image2-prompts/tree/main` 提示词数据库；优先解析其公开提示词备份和页面数据，写入远程提示词来源，保留 `sourceRepo = davidwuw0811-boop/awesome-gpt-image2-prompts`、`remoteId`、`githubUrl`、分类、标签和封面图。
- 同步后执行 hash/simhash 去重和 AI 重复审核；不覆盖已有优质提示词。
- 保持反单文件治理：榜单页面、图像导入、公开标题、提示词源 parser 等逻辑拆到独立模块或既有服务模块，不继续把大段实现堆进 `public/app.js` / `server.js`。

验收：

- 画廊卡片中的图片能实际显示；若远程图片不可访问，页面显示明确 fallback，不能大面积灰块。
- 榜单从画廊主栅格中移出，独立页面可访问、可切换榜单范围、可进入详情、可点赞。
- 公开图片时可填写标题；不填写时自动生成标题；画廊卡片、详情、榜单和后台都展示同一标题。
- 对话列表中每条对话可删除，刷新后仍保持删除状态；删除操作不误删公开作品。
- 对话列表中每条对话可编辑标题，刷新后仍保持标题。
- 生成中轮询不再重建整段对话 DOM，不出现整段闪烁。
- admin 的普通历史接口只返回自己的历史；后台用户管理可通过明确按钮查看某个用户的对话记录。
- 对话列表下方提示词输入框在桌面端宽度收敛，移动端不溢出。
- 图生图支持文件选择、粘贴和拖拽三种导入方式，预览一致，错误提示一致。
- `/api/prompts?includeHidden=1` 可看到 `sourceRepo = davidwuw0811-boop/awesome-gpt-image2-prompts` 的提示词；前台可搜索、打开详情、复制、点赞、使用、加入画布，并且封面图可展示或有明确 fallback。

### AIS-RLS-047：画布工作台布局与连线系统重构

优先级：P0
标签：`frontend`, `canvas`, `layout`, `edge-routing`, `qa`, `architecture`
依赖：`AIS-RLS-039`, `AIS-RLS-046`
建议状态：Backlog

目标：

- 修复画布工作台的视口居中、右侧检查器遮挡、工具栏文字竖排和节点连线不贴合端口等核心体验问题，参考原无限画布项目 `https://github.com/basketikun/infinite-canvas` 以及成熟画布/节点编辑器项目的坐标系与 edge routing 思路，对现有画布实现做可维护重构。

交付物：

- 重新梳理画布工作台布局：左侧项目/节点栏、中心画布、顶部工具栏、右侧检查器/Assistant 面板在桌面和移动端都不互相遮挡。
- 重构节点端口坐标和连线渲染逻辑，确保拖动节点、缩放、平移、选择、多选后连线端点仍贴合输出/输入端口。
- 明确右侧检查器和 Assistant 面板的宽度、层级、折叠或响应式策略，不再覆盖画布工具和交互热区。
- 验证既有画布能力：项目列表、创建/保存、节点增删、拖拽、连线、撤销重做、复制粘贴、导入导出、小地图、助手、模板市场、生成、发布到广场，以及从首页/画廊/作品加入画布。
- 保持反单文件治理：坐标系、连线渲染、布局尺寸、检查器/侧栏适配和测试辅助拆到独立模块或既有 `canvas-*` 模块，不把重构集中堆进 `public/canvas.js` 或 `public/app.js`。

验收：

- 画布主视口位于工作区中央，不被左栏和右侧检查器遮挡；工具栏按钮文字不竖排、不互相覆盖。
- 任意节点连接后，连线端点贴合输出/输入端口；拖动节点、缩放、平移后连线实时更新且不漂移。
- 右侧检查器和 Assistant 面板不会盖住画布交互热区；窄屏时有明确折叠或堆叠策略。
- 画布所有核心 smoke 测试通过，并新增专项测试覆盖布局居中、检查器不遮挡和连线端点对齐。
- 从首页/画廊/提示词/我的作品加入画布后，节点、连线、生成和发布链路仍与现有功能兼容。
